...
def _current_json(path: Path, release: str, digest: str) -> bool:
    try:
        ...
        return False
    if "release" in payload and payload.get("release") != release: return False
...
def claim_admission_checks(root: Path, release: str, digest: str) -> dict[str, bool]:
    ...
    for claim in load_object(ledger).get("claims", ()):
        if not isinstance(claim, dict) or not str(claim.get("status", "")).startswith("SUPPORTED"): continue
        evidence = claim.get("evidence")
        if not isinstance(evidence, str) or not evidence: unresolved += 1; continue
        target = root / evidence
        if not target.is_file() or (target.suffix == ".json" and not _current_json(target, release, digest)):
            unresolved += 1
    return {"CLAIM_LEDGER.supported_evidence_resolves": unresolved == 0, "CLAIM_LEDGER.supported_unresolved_zero": unresolved == 0}
...
def blocker_state_checks(root: Path, release: str, digest: str) -> dict[str, bool]:
    ...
    return {
        ...
        "BLOCKER_REGISTRY.hard_predicate_report_current": payload.get("hard_predicate_report_current") is True,
        "BLOCKER_REGISTRY.internal_executable_unresolved_zero": payload.get("internal_executable_unresolved") == 0,
