...
def sign(manifest: Path, key: Path, out: Path) -> int:
    ...
    print(
        json.dumps(
            {
                k: payload[k]
                for k in ("release", "manifest_sha256", "public_key_sha256", "trust_class")
            },
            indent=2,
        )
    )
...
def _trusted(path: Path | None, field: str) -> set[str]:
    ...
    return {str(value) for value in payload.get(field, ())}
...
def verify(
    ...
    *,
    ...
    trust_field: str = "release_ed25519_public_key_sha256",
    trust_env: str | None = None,
        ...
        json.dumps(
            {
                ...
                "checks": checks,
                "failures": failures,
                "public_key_sha256": verdict.fingerprint,
            },
            indent=2,
        )
    )
...
def main() -> int:
    ...
    signer.add_argument("--manifest", type=Path, required=True)
    ...
    verifier.add_argument("--attestation", type=Path, required=True)
    verifier.add_argument("--trust-config", type=Path)
    verifier.add_argument("--trust-field", default="release_ed25519_public_key_sha256")
    verifier.add_argument("--trust-env")
    verifier.add_argument("--require-trusted", action="store_true")
    args = parser.parse_args()
    return (
        sign(args.manifest, args.key, args.out)
        if args.command == "sign"
        else verify(
            args.manifest,
            args.attestation,
            trust_config=args.trust_config,
            trust_field=args.trust_field,
