...
def verify(root: Path) -> dict[str, object]:
    ...
    authoritative = [p.as_posix() for p in source_paths(root)] if (root / ".git").exists() else sorted(by_path)
    failures = manifest_failures(manifest, records)
    if sorted(by_path) != authoritative:
        failures.append(f"path parity mismatch missing={sorted(set(authoritative)-set(by_path))} extra={sorted(set(by_path)-set(authoritative))}")
    for relative in authoritative:
        ...
        if not file.is_file():
            ...
            failures.extend(f"source {item}" for item in record_failures(file, record, mode_string(file, source=True)))
    return {"valid": not failures, "files": len(authoritative), "root_sha256": manifest.get("root_sha256"), "failures": failures}


def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--root", type=Path, default=ROOT); args = parser.parse_args()
    payload = verify(args.root.resolve()); print(json.dumps(payload, indent=2))
