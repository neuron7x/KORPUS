...
def _https_get(url: str, timeout: float) -> tuple[int, dict[str, str], bytes]:
    request = urllib.request.Request(url, method="GET", headers={"User-Agent": "korpus-production-smoke/1"})
    context = ssl.create_default_context()
    with urllib.request.urlopen(request, timeout=timeout, context=context) as response:
        return response.status, {k.lower(): v for k, v in response.headers.items()}, response.read(1_048_576)
...
def evaluate(domain: str, expected_ip: str, timeout: float = 15.0) -> list[Check]:
    ...
    try:
        ...
        checks.append(Check("DNS_EDGE_BINDING", dns_ok, f"resolved={addrs}; expected={expected_ip}"))
    ...
    try:
        ...
        checks.append(Check("WEB_HEALTH", code == 200 and b"ok" in body.lower(), f"status={code}; body={body[:200]!r}"))
        required = {
    ...
    try:
        ...
        checks.append(Check("API_NO_SHARED_CACHE", "no-store" in cache_control or "private" in cache_control, f"cache-control={cache_control!r}"))
    except (urllib.error.URLError, TimeoutError, ssl.SSLError, OSError) as exc:
    ...
    try:
        ...
        checks.append(Check("HTTP_REDIRECT", False, "HTTP unexpectedly returned success without redirect"))
    except urllib.error.HTTPError as exc:
        location = exc.headers.get("Location", "")
        checks.append(Check("HTTP_REDIRECT", exc.code in {301, 302, 307, 308} and location.startswith(f"https://{domain}"), f"status={exc.code}; location={location}"))
