...
def evaluate(s: object) -> list[tuple[str, bool, str]]:
    ...
    return [
        ...
        (
            ...
            and workflow.find('Execute exact private candidate probe') < workflow.find('Promote candidate to bounded canary traffic') < workflow.find('Admit candidate by revision metrics') < workflow.find('Promote verified candidate to 100 percent'),
