...
# `export VAR="$(cmd)"` НЕ переривається під `set -e`: перевіряється статус
# `export`, який успішний завжди. `governance_value` падає на відсутньому ключі
# з KeyError — і в Terraform ішов ПОРОЖНІЙ sha256 профілю врядування, а деплой
# продовжувався. Неправильне значення тут не відрізнити від відсутнього.
#
# Звичайне присвоєння `v="$(cmd)"` під `set -e` перериває скрипт (перевірено:
# функція з `return 3` дає rc=3). Плюс явна перевірка на порожнє: JSON може
# віддати "" законно, і тоді `set -e` мовчить.
for pair in \
  "TF_VAR_entitlement_profile_sha256:entitlements.json" \
  "TF_VAR_source_trust_profile_sha256:source-trust.json" \
  "TF_VAR_reviewer_registry_sha256:reviewers.json" \
  "TF_VAR_corpus_governance_profile_sha256:corpus-governance.json" \
  "TF_VAR_calibration_profile_sha256:calibration.json"
do
  var="${pair%%:*}"
  file="${pair#*:}"
  value="$(governance_value "$file")"
  [[ -n "$value" ]] || { printf 'governance digest for %s is empty\n' "$file" >&2; exit 65; }
  export "${var}=${value}"
done
