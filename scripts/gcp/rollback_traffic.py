...
def rollback_service(*, project: str, region: str, service: str, spec: str, evidence: Path) -> bool:
    ...
    if not spec:
        evidence.write_text(json.dumps({"service": service, "rollback": "not-applicable"}) + "\n", encoding="utf-8")
        return True
    ...
    result = subprocess.run(
        [
            ...
            "--to-revisions", canonical, "--format=json",
        ],
        check=False,
        capture_output=True,
        text=True,
    ...
    if result.returncode == 0:
        try:
            ...
            payload = {"service": service, "rollback": "invalid-gcloud-json", "stdout": result.stdout[:4096]}
    ...
    error.write_text(
        json.dumps({"service": service, "returncode": result.returncode, "stderr": result.stderr[-4096:]}, sort_keys=True, indent=2) + "\n",
        encoding="utf-8",
    )
    return False
...
def main() -> int:
    ...
    for service, spec, filename in (
        ...
        try:
            outcomes.append(rollback_service(project=args.project, region=args.region, service=service, spec=spec, evidence=args.output_dir / filename))
        except (ValueError, OSError) as exc:
            (args.output_dir / f"{service}-rollback.error.json").write_text(
                json.dumps({"service": service, "error": type(exc).__name__, "detail": str(exc)}, sort_keys=True) + "\n",
                encoding="utf-8",
            )
