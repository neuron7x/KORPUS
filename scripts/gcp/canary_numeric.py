...
def request_count(value: object) -> int:
    if _strict_int(value):
        if value < 0: raise ValueError("request_count cannot be negative")
        return value
    if isinstance(value, str) and value.isascii() and value.isdecimal(): return int(value)
...
def validate_timing(window_seconds: object, wait_seconds: object, poll_seconds: object) -> None:
    ...
    if window_seconds < 60 or wait_seconds < 0 or poll_seconds < 1: raise ValueError("invalid metric timing policy")
