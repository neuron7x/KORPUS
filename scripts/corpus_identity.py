...
Тут рахується ідентичність САМОГО корпусу: голова журналу подій, кількість документів,
версій і прольотів, і сума хешів прольотів. Голова змінюється від кожної записаної події,
сума хешів — від кожного зміненого символу цитати. Разом вони рухаються тоді й тільки
тоді, коли рухається те, про що звітують осі.
...
def corpus_identity(database: Path) -> dict[str, object]:
    ...
    return {
        ...
        "spans": int(spans),
        "audit_head": str(head[0]) if head else None,
