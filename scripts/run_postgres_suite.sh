...
if [[ -n "${KORPUS_TEST_DATABASE_URL:-}" ]]; then
  if [[ -z "${KORPUS_REVIEW_DATABASE_URL:-}" || -z "${KORPUS_AUTHZ_DATABASE_URL:-}" ]]; then
    echo "KORPUS_REVIEW_DATABASE_URL and KORPUS_AUTHZ_DATABASE_URL are required with an external PostgreSQL test DB" >&2
    exit 2
  fi
...
KORPUS_DATABASE_URL="$admin_url" KORPUS_POSTGRES_ADMIN_URL="$admin_url" \
  ...
  KORPUS_POSTGRES_AUTHZ_ROLE=korpus_authz \
