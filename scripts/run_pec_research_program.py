...
from korpus.application.pec_research import (
    ...
    feature_ablation_generalization,
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", type=Path, default=ROOT / "reports/PEC_RESEARCH_PROGRAM_CURRENT.json"
    )
...
def _oracle_training_rows(dataset: list[dict], oracle: dict) -> list[TrainingRow]:
    ...
    for decision in oracle.get("decisions", []):
        ...
        if (
            decision.get("oracle_status") != "PASS"
            or meta is None
            or meta.get("partition") != "train"
        ):
            continue
        output.append(
            TrainingRow(
...
def main() -> int:
    ...
    conditional = (
        conditional_risk_report(
            risk_rows,
            stratum_key="risk_class",
            error_key="answer_error",
            risk_limit=args.risk_limit,
            delta=args.delta,
            minimum_samples=args.minimum_stratum_samples,
        )
        if risk_rows
        else {"status": "UNKNOWN", "reason": "no_replay_risk_rows"}
    )
    ...
    priority = (
        replay_priority_enrichment(replay_rows)
        if replay_rows
        else {"status": "UNKNOWN", "reason": "no_replay_rows"}
    )
    information_gain = (
        observed_information_gain(replay_rows)
        if replay_rows
        else {"status": "UNKNOWN", "reason": "no_replay_rows"}
    )
    ...
    report = receipt(
        "pec_research_program",
        {
            "status": status,
            "scientific_authority": "PRODUCTION_JUDGED" if scientific_authority else "NONE",
            "dataset_sha256": sha256_file(args.dataset),
            "replay_sha256": replay_digest,
            "oracle_sha256": oracle_digest,
            "production_judgment_validity": validity,
            "conditional_risk": conditional,
            "feature_generalization": generalization,
        ...
        json.dumps(
            {
                ...
                if k not in {"feature_generalization", "observed_information_gain"}
            },
            indent=2,
        )
    )
