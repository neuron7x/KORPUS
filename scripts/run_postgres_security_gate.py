...
def main() -> int:
    static = subprocess.run(
        [sys.executable, "-m", "pytest", "-q", "--disable-warnings", "apps/api/tests/test_postgres_role_grants.py"],
        cwd=ROOT, env={**os.environ, "PYTHONPATH": str(ROOT / "apps/api/src")}, capture_output=True, text=True, check=False, timeout=120,
    )
    ...
    if external_url or docker:
        completed = subprocess.run(
            ["bash", "scripts/run_postgres_suite.sh", *TARGETS], cwd=ROOT,
    ...
    result = gate_payload(
        "postgres_security", status="PASS" if not failures else "FAIL",
        source_digest=compute_source_digest(ROOT), release=release_tag(), checks=checks,
        failures=failures, backend="postgresql" if runtime_exit == 0 else "UNEXECUTED",
        evidence_class="REAL_POSTGRESQL_REQUIRED", pytest_targets=TARGETS,
        runtime_exit_code=runtime_exit, runtime_tail=runtime_tail,
