...
def main() -> int:
    ...
    try:
        ...
        checks = {
            ...
            "request_id_not_metric_label": all(
                "request_id" not in labels for labels in collectors.values()
            ),
            "telemetry_state_explicit": obs.telemetry_status().get("traces")
            in {"DISABLED", "ACTIVE", "REQUESTED_NOT_ACTIVE"},
        }
    finally:
        obs.close()
    ...
    result = gate_payload(
        ...
        checks=checks,
        failures=failures,
        metric_labels=collectors,
        forbidden_label_hits=leaked,
