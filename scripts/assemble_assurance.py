...
def main() -> int:
    ...
    checks = {
        ...
        "mutation": loaded["mutation"].get("mutation_score") == 1.0,
    ...
    report = {
        ...
        "checks": checks,
        "pytest": {key: suite.attrib.get(key, "0") for key in ("tests", "failures", "errors", "skipped", "time")},
        "coverage": {
    ...
    print(json.dumps({"status": report["status"], "checks": checks, "source_tree_sha256": report["source_tree_sha256"]}, indent=2))
