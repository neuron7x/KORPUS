...
def summarise_inflection(results: list[dict[str, Any]], unreachable: int) -> dict[str, Any]:
    ...
    return {
        ...
        #: Поля `ceiling` тут БІЛЬШЕ НЕМАЄ, і це виправлення власної помилки. Воно
        #: рахувало 12/14 — число, виведене з ЗАМИКАННЯ СУФІКСІВ, тобто з методу, який
        #: я зрештою не взяла. Обраний шлях (збіг за початком слова) його не має: на
        #: копії він дав 13/14. Твердження про межу, успадковане від покинутого підходу,
        #: читалось би як «уперлись у стелю» рівно тоді, коли вимір її досяг. Межу
        #: тепер називають ДАНІ — `failed_in_genitive` перелічує тих, хто не влучив.
