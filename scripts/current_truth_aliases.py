...
def alias_checks(root: Path, release: str) -> dict[str, bool]:
    ...
    for name in ("GITHUB_IMPORT.md", "GITLAB_IMPORT.md"):
        ...
        if path.is_file():
            ...
            checks[f"{name}.release_bound"] = release in text
            checks[f"{name}.artifact_bound"] = bool(artifact) and artifact in text
    package_build = root / "PACKAGE_BUILD.json"
    checks["PACKAGE_BUILD.present"] = package_build.is_file()
    if package_build.is_file():
        checks["PACKAGE_BUILD.release_bound"] = load_object(package_build).get("release") == release
    return checks
