...
def main() -> int:
    ...
    for decision in decisions:
        stable_baseline += int(
            decision.get("oracle_reason") == "baseline_decision_already_admissible"
        )
    ...
    status = (
        "PASS"
        if decisions and nonbaseline_without_value == 0
        else ("FAIL" if nonbaseline_without_value else "UNKNOWN")
    )
    report = receipt(
        "pec_decision_sensitivity",
        {
            "status": status,
            ...
            "boundary_distance": {
                ...
                "mean": sum(boundary_distances) / len(boundary_distances)
