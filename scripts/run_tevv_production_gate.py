...
def _deployment_checks(
    evidence: dict[str, Any], profile: dict[str, Any], policy: dict[str, int | float | bool]
) -> dict[str, bool]:
    required = set(profile["required_simulated_dependency_failures"])
    simulated = evidence.get("simulated_dependency_failures", [])
    simulated_set = set(simulated) if isinstance(simulated, list) else set()
    return {
        "deployment_context_bound": not policy["deployment_simulation_required"]
        or _sha256(evidence.get("deployment_context_sha256")),
        "evaluation_cues_blinded": not policy["evaluation_cue_blinding_required"]
        or evidence.get("evaluation_cues_blinded") is True,
        "dependency_failures_simulated": not policy["dependency_failure_simulation_required"]
        or required.issubset(simulated_set),
    }
...
def evaluate(
    ...
    total, passed = metrics["observations"], metrics["passed"]
    tevv = evaluate_tevv(
        passed=passed,
        total=total,
        corpus_declaration=evidence.get("corpus"),
        maximum_interval_width=policy["maximum_interval_width"],
        minimum_observations=policy["minimum_observations"],
    )
    attested, attestation_verdict = attestation_checks(
        evidence_bytes, manifest_name, release, attestation, trusted, "assessor"
    )
    checks = {
        "evidence_schema": evidence.get("schema") == profile["evidence_schema"],
        "preregistered": evidence.get("preregistration_sha256")
        ...
        in profile["allowed_environment_classes"],
        "independent_class": evidence.get("evidence_class") == profile["required_evidence_class"],
        "assessor_structured": assessor_identity_valid(evidence),
        **attested,
        **ledger["checks"],
        "tevv_admissible": tevv.admissible,
        ...
        "attack_families": set(profile["required_attack_families"]).issubset(
    ...
    return gate_payload(
        ...
        failures=failures,
        environment_class=evidence.get("environment_class"),
        assessor_signer_fingerprint=attestation_verdict.fingerprint,
        tevv=tevv.as_dict(),
        observations=total,
