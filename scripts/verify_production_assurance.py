...
def _gate_state() -> tuple[dict[str, dict], dict[str, str]]:
    ...
    return ({gate: _json(path) for gate, path in paths.items()},
            {gate: hashlib.sha256(path.read_bytes()).hexdigest() for gate, path in paths.items() if path.is_file()})
...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--report", type=Path, default=ROOT / "reports/PRODUCTION_ASSURANCE_REPORT.json"); parser.add_argument("--attestation", type=Path, default=ROOT / "reports/PRODUCTION_ASSURANCE_REPORT.attestation.json"); args = parser.parse_args()
    checks = _checks(args.report, args.attestation); failures = [name for name, ok in checks.items() if not ok]
    print(json.dumps({"valid": not failures, "checks": checks, "failures": failures}, indent=2)); return int(bool(failures))
