...
def main() -> int:
    ...
    chaos, load, recovery = (
        _read(ROOT / "var/chaos-matrix.json"),
        _read(load_path),
        _read(recovery_path),
    )
    source, release = compute_source_digest(ROOT), release_tag()
    trusted = trusted_fingerprints(
        TRUST, "environment_ed25519_public_key_sha256", "KORPUS_TRUSTED_ENVIRONMENT_SIGNER_SHA256"
    )
    checks, load_fp, recovery_fp = evaluate_attested_reliability(
        ...
        recovery_attestation=_read(ROOT / "var/production/recovery-report.attestation.json"),
        trusted=trusted,
    )
    ...
    result = gate_payload(
        ...
        checks=checks,
        failures=failures,
        evidence_class="ATTESTED_LIVE_PLUS_FAULT_INJECTION_REQUIRED",
        load_metrics={name: load.get(name, {}) for name in ("load", "spike", "soak")},
        chaos_cases=len(chaos.get("cases", ())),
        recovery_status=recovery.get("status", "NOT_EXECUTED"),
        load_signer_fingerprint=load_fp,
        recovery_signer_fingerprint=recovery_fp,
    )
    out = ROOT / "var/production/reliability-gate.json"
