...
DEFAULT_GATES = {
    ...
    # незалежного оцінювача немає (модель урядування, 03.09.2026). Клас доказу видно в
    # самому артефакті: INTERNAL_ADVERSARIAL.
    #
    # ПОПЕРЕДНЯ РЕДАКЦІЯ ЦЬОГО КОМЕНТАРЯ БУЛА ХИБНА ПРО ЦЕ ДЕРЕВО. Вона казала, що
    # `redteam-gate.json` «лишається порожнім назавжди». Виміряно 06.09.2026: файл
    # існує, перезнімається кампанією разом з рештою, несе ПОТОЧНИЙ дайджест і
    # `status: FAIL` із 13 перевірками — тобто він не порожній, а червоний, і червоний
    # чесно: зовнішнього redteam справді не проводили.
    # Рішення ігнорувати його лишається правильним, підстава — ні: ми не читаємо його
    # тому, що предикат вимагає INTERNAL_ADVERSARIAL_CAMPAIGN, а не тому, що там нічого
    # немає. Неправдива підстава небезпечніша за неправильне рішення: вона переживає
    # рішення і виправдовує наступні.
