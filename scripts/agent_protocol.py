...
#: «Які інструменти має цей агент» — функція, яку самоперевірка підміняє,
#: тож тип оголошено псевдонімом, а не привʼязано до `agent_tools`.
ToolsOf = Callable[[str], set[str] | None]
...
def _unique_agents(agents: list[dict]) -> list[str]:
    ...
    return [
        f"агент {name!r} оголошений {n} разів"
        for name, n in Counter(a.get("agent") for a in agents).items()
        if n > 1
    ]
...
def _one_agent_per_axis(agents: list[dict]) -> list[str]:
    ...
    return [
        f"вісь {axis!r} на даних {scope!r} зайнята кількома агентами "
        f"({', '.join(who)}) — другий не приносить нової осі, це витрата"
        for (axis, scope), who in sorted(pairs.items())
        if len(who) > 1
    ]
...
def _instrument_described(a: dict) -> list[str]:
    ...
    if len((a.get("instrument") or "").strip()) < MIN_INSTRUMENT_CHARS:
        return [
            f"{a.get('agent')}: `instrument` коротший за {MIN_INSTRUMENT_CHARS} "
            f"символів — назва ролі замість опису інструмента"
        ]
    return []


def _acceptance_basis(a: dict, tools_of: ToolsOf) -> list[str]:
    ...
    if basis not in {"no-write-tools", "different-vantage"}:
        return [
            f"{a.get('agent')}: приймальник без оголошеної підстави "
    ...
    if basis == "different-vantage":
        if len((a.get("vantage") or "").strip()) < MIN_INSTRUMENT_CHARS:
            return [
                f"{a.get('agent')}: підстава `different-vantage` без опису "
    ...
    if tools & WRITE_TOOLS:
        return [
            f"{a.get('agent')}: приймальник із правом запису "
...
def _residual_declared(a: dict, tools: set[str]) -> list[str]:
    ...
    if residual and declared != residual:
        bad.append(
            f"{a.get('agent')}: має залишкові шляхи запису {residual}, "
    ...
    if declared and not residual:
        bad.append(
            f"{a.get('agent')}: оголошено залишкові шляхи {declared}, "
            f"яких у агента немає — оголошення описує не цього агента"
        )
    return bad


def check_registry(reg: dict, tools_of: ToolsOf = agent_tools) -> list[str]:
...
def _verdicts(records: list[dict], claims: dict, vocab: dict) -> tuple[set[str], list[str]]:
    ...
    for r in records:
        ...
        if word not in settles | opens:
            ...
            bad.append(
                f"вирок {r.get('id')}: слово {word!r} поза словником "
        ...
        if r.get("actor") == claim.get("actor") and word in raising:
            ...
            bad.append(
                f"{r.get('id')}: виробник {claim.get('actor')} ПІДНІМАЄ власне "
...
def _claim_budget(claims: dict, settled: set[str], budget: int) -> list[str]:
    ...
    return [
        f"актор {actor}: {n} тверджень без вироку при бюджеті {budget} — "
        f"домогтися вироку по старих, перш ніж додавати нові"
        for actor, n in sorted(open_by_actor.items())
        if n > budget
    ]
...
def check_plan(reg: dict, plan: dict) -> list[str]:
    ...
    if mode == "produce" and len(axes) < 2:
        bad.append(
            f"план покриває {len(axes)} вісь — це один агент за ціною кількох; "
            f"потрібні щонайменше дві РІЗНІ осі"
        )
    if mode == "adjudicate" and producers:
        bad.append(
            f"режим adjudicate нічого не виробляє, але оголошено виробників "
...
def selftest() -> int:
    ...
    def base_reg(**over: Any) -> dict:
        reg = {
            "verdict_vocabulary": {
                "settles": ["ACCEPTED", "REFUTED"],
    ...
    def claim(cid: str, actor: str, ev: int = 2) -> dict[str, Any]:
        return {"kind": "claim", "id": cid, "actor": actor, "claim": "x", "evidence": ["е"] * ev}

    def verdict(cid: str, actor: str, word: str = "ACCEPTED") -> dict[str, Any]:
        return {"kind": "verdict", "id": cid, "actor": actor, "verdict": word}

    def tools_ok(n: str) -> set[str]:
        return {"Read", "Bash"} if n == "acc" else {"Read", "Write"}

    def tools_writer(n: str) -> set[str]:
        return {"Read", "Write"}

    def tools_missing(n: str) -> set[str] | None:
        return None

    cases: list[tuple[str, bool, list[str]]] = [
        ("реєстр як є — зелений", False, check_registry(base_reg(), tools_ok)),
        (
            "дві осі однакові — червоний",
            True,
            check_registry(
                base_reg(
                    agents=[
        ...
        (
            "та сама вісь на ІНШИХ даних — зелений",
            False,
            check_registry(
                base_reg(
                    agents=[
        ...
        (
            "інструмент — лише назва ролі — червоний",
            True,
            check_registry(
                base_reg(
                    agents=[
        ...
        ("приймальник із правом запису — червоний", True, check_registry(base_reg(), tools_writer)),
        (
            "Bash без оголошення — червоний",
            True,
            check_registry(
                base_reg(
                    agents=[
        ...
        (
            "оголошено те, чого немає — червоний",
            True,
            check_registry(
                base_reg(
                    agents=[
        ...
        (
            "приймальника без опису агента — червоний",
            True,
            check_registry(base_reg(), tools_missing),
        ),
        (
            "приймальник без оголошеної підстави — червоний",
            True,
            check_registry(
                base_reg(
                    agents=[
        ...
        (
            "сесія з іншою точкою і описом — зелений",
            False,
            check_registry(
                base_reg(
                    agents=[
                        {
                            ...
                            "instrument": "о" * 40,
                            "can_accept": True,
                            "accept_basis": "different-vantage",
                            "vantage": "т" * 40,
                        }
                    ]
                ),
        ...
        (
            "сесія з іншою точкою БЕЗ опису точки — червоний",
            True,
            check_registry(
                base_reg(
                    agents=[
        ...
        (
            "журнал як є — зелений",
            False,
            check_ledger(base_reg(), [claim("c1", "prod"), verdict("c1", "acc")]),
        ),
        (
            "виробник ПРИЙМАЄ себе — червоний",
            True,
            check_ledger(base_reg(), [claim("c1", "prod"), verdict("c1", "prod", "ACCEPTED")]),
        ),
        (
            "виробник СПРОСТОВУЄ себе — зелений",
            False,
            check_ledger(base_reg(), [claim("c1", "prod"), verdict("c1", "prod", "REFUTED")]),
        ),
        (
            "слово поза словником — червоний",
            True,
            check_ledger(base_reg(), [claim("c1", "prod"), verdict("c1", "acc", "ACCEPTEDD")]),
        ),
        (
            "вирок без твердження — червоний",
            True,
        ...
        (
            "бюджет вичерпано — червоний",
            True,
            check_ledger(base_reg(), [claim(f"c{i}", "prod") for i in range(3)]),
        ),
        (
            "бюджет звільнено вироками — зелений",
            False,
            check_ledger(
                base_reg(),
                [claim(f"c{i}", "prod") for i in range(3)]
        ...
        (
            ...
            check_plan(
                ...
                {"producers": ["prod", "prod2"], "acceptor": "acc"},
            ),
        ),
        (
            "одна вісь у плані — червоний",
            True,
            check_plan(base_reg(), {"producers": ["prod"], "acceptor": "acc"}),
        ),
        (
            "adjudicate без виробників — зелений",
            False,
            check_plan(base_reg(), {"mode": "adjudicate", "producers": [], "acceptor": "acc"}),
        ),
        (
            "adjudicate З виробниками — червоний",
            True,
            check_plan(
                base_reg(), {"mode": "adjudicate", "producers": ["prod"], "acceptor": "acc"}
            ),
        ),
        (
            "режим поза словником — червоний",
            True,
            check_plan(base_reg(), {"mode": "думати", "producers": ["prod"], "acceptor": "acc"}),
        ),
        (
            "приймальник він же виробник — червоний",
            True,
            check_plan(
                base_reg(
                    agents=[
                ...
                {"producers": ["prod", "p2"], "acceptor": "prod"},
            ),
        ),
        (
            "виробник поза реєстром — червоний",
            True,
            check_plan(base_reg(), {"producers": ["prod", "хтось"], "acceptor": "acc"}),
        ),
        (
            "приймальник не оголошений таким — червоний",
            True,
            check_plan(base_reg(), {"producers": ["prod", "acc"], "acceptor": "prod"}),
        ),
    ]
    bad = 0
