...
.PHONY: pilot-posture machine-tevv release-freeze ci-run ci-mirror assurance-model assurance-model-selftest production-exact-environment-image mcp-serve corpus-axes evidence-bases dormant-subsystems control-copy serving-freshness lane-report branch-consolidation install-nightly-gates check-nightly nightly-load-probe nightly-evidence check-deployment deployment-debt deployment-debt-selftest public-env-parity public-env-parity-selftest gate-closure gate-closure-selftest public-surface public-surface-selftest subject-precision repair-span-markup fetch-stubs diagnose-retrieval span-hygiene compare-retrieval remap-reference-versions serve-semantic-local restore-document-types embedding-backfill-sqlite runtime-corpus-manifest refusal-retryability publication-mirrors agent-protocol catalog-uri-uniqueness cache-in-tree evidence-refusal gate-liveness capture-evidence capture-evidence-selftest content-signals remote-digest document-probe deterministic-replay provenance provenance-verify reference-set reference-eval embedding-candidate-screen embedding-backfill corpus-admission gold-annotation-audit runtime-corpus-audit service-objectives corpus-release corpus-release-verify security-scan reproducible-build chaos-matrix ingestion-drill load-probe backup-sqlite restore-sqlite attest-restore drive-snapshot drive-public serve-public public-tunnel draft-manifest import-corpus review-token audit-export web-contract web-contract-check environment-drift environment-observe requirements-register module-budget file-modes import-cycles release-identity source-manifest-verify corpus-path-declarations document-references diversify-benchmark declared-metrics retention-plan postgres-suite postgres-coverage sqlite-recovery-drill recover-corpus admission-differential adjudication-properties corruption-repro handoff-verify handoff-verify-bound openapi audit-closure desired-state supply-chain-inventory kubernetes-validate github-actions-validate infra-validate backup-postgres restore-postgres api-install api-run api-test api-lint web-install web-run web-build bootstrap eval mutation migration-gate scale operational-gate assurance assemble-assurance snapshot audit-verify validate check release infra-secrets infra-up infra-support infra-down package clean production-engineering production-tevv production-observability production-state-contracts production-authorization production-redteam-internal production-redteam-external production-inference-security production-reliability-internal production-reliability production-postgres-security production-exact-environment production-sbom production-supply-chain production-mutation production-assurance production-assurance-verify production-release dependency-locks assurance-model-check standards-control-map bibliography bibliography-check slsa-provenance slsa-provenance-verify release-mutation-delta package-build-identity evidence-refresh mutation-probe mutation-report-freshness evidence-freshness release-surface release-verify release-verify-closure answer-quality answer-axes corpus-integrity corpus-readability corpus-heal recut-spans coverage-ratchet coverage-union determinism-gate stress-gate plasticity-gate canonical-release-cycle production-hard-predicates military-readiness military-readiness-full evidence-stores selftest-coverage selftest-falsifiability installed-units-verify canonical-verify branch-integration owner-packet owner-packet-check mutation-delta-gate mutation-delta-gate-selftest blank-corpus-probe clean-room serving-readiness serving-readiness-selftest evidence-epistemics evidence-epistemics-selftest corpus-pack corpus-install corpus-transport-rehearsal corpus-transport-selftest

api-install:
	python3 -m venv apps/api/.venv
	$(PIP) install --no-deps --require-hashes --requirement apps/api/requirements.dev.lock

# КОРПУС як інструмент агента. Тонкий перехідник над власним HTTP-API: у процесі
# агента немає ні журналу, ні прив'язки RLS, тож другий шлях відповіді там був би
# відповіддю без сліду й без межі.
mcp-serve:
	MCP_KORPUS_BASE_URL="$(or $(BASE_URL),http://127.0.0.1:8000)" \
	MCP_KORPUS_TOKEN="$(TOKEN)" $(PY) scripts/run_mcp_server.py
...
# Другий діалект — із ПОКРИТТЯМ. `postgres-suite` іде з --no-cov навмисно (він міряє
# поведінку), тож `var/coverage-postgres.json` мав рівно одного виконавця — джоб у
# `.gitlab-ci.yml`. Виміряно 08.09.2026: після будь-якої зміни джерела `coverage-union`
# порівнював свіжий звіт SQLite зі СТАРИМ звітом постгреса й чесно відмовляв, а
# полагодити це локально не було чим. Відмова читається як регрес покриття, хоча каже
# «тут нема виробника».
postgres-coverage:
	scripts/run_postgres_coverage.sh
...
## Дефолт, який не існує, означає, що безаргументна дорога інструмента МЕРТВА, і
## виявиться це в найгіршу мить. Виміряно 02.09.2026: три дефолти вказували на
## неіснуючий var/korpus-ml.db, і один із них — бекап живого корпусу на 276 МБ.
corpus-path-declarations:
	$(PY) scripts/check_corpus_path_declarations.py --selftest
	PYTHONPATH=apps/api/src $(PY) scripts/check_corpus_path_declarations.py --out var/corpus-path-declarations.json

## Ізольований відтворюваний вимір `diversify_evidence`: дві редакції в ОДНОМУ процесі,
## чергуванням A,B,A,B — інакше зміна коду не відрізняється від дрейфу машини. Виміряно
## 02.09.2026: послідовне порівняння «показало» +3..6 % від виносу згортання, чергування
## показало -0.3 / -26.3 / -35.8 % і тотожний вихід. Висновок був би зроблений із шуму.
diversify-benchmark:
	$(PY) scripts/bench_diversify.py --selftest
	PYTHONPATH=apps/api/src $(PY) scripts/bench_diversify.py --out var/diversify-benchmark.json

## Оголошена метрика, яку ніхто не наповнює, — не вимір, а порожнє місце з іменем.
## Виміряно 02.09.2026: з дванадцяти оголошених метрик ДВІ не наповнювались, і через це
## три конкурентні пояснення хвостової затримки давали ОДНАКОВІ спостереження. Порожній
## ряд у Prometheus не відрізняється від системи без навантаження.
declared-metrics:
	$(PY) scripts/check_declared_metrics_are_observed.py --selftest
	$(PY) scripts/check_declared_metrics_are_observed.py --out var/declared-metrics.json

## Документ, що називає файл, якого немає, — не помилка форматування. Виміряно
## 02.09.2026: звіт про стадію руйнування подавав числа від ТРЬОХ скриптів атак,
## яких не існувало в жодній гілці. Помічене подавалось як виміряне.
document-references:
	$(PY) scripts/check_document_references.py --selftest
	$(PY) scripts/check_document_references.py --out var/document-references.json
...
bibliography:
	$(PY) scripts/verify_bibliography.py --write --selftest

bibliography-check:
	$(PY) scripts/verify_bibliography.py --selftest
...
# Пакет власника більше не тримається рукою. Клас вади названий у §16 самого пакета:
# документ розійшовся з виміром тричі за півдня, і гейт `source_bound` цього не бачив —
# він звіряє ДАЙДЖЕСТ, а не те, чи числа під ним про це дерево.
owner-packet:
	PYTHONPATH=apps/api/src $(PY) scripts/refresh_owner_packet.py

owner-packet-check:
	PYTHONPATH=apps/api/src $(PY) scripts/refresh_owner_packet.py --check

# Контур, що застосовується сам. Ратчет покриття не дає новому коду лишитись
# невиконаним, курована мутація тримає 624 мутанти — але обидва мовчать про модуль,
# якого в каталозі немає, а таких 162 (42% джерела, виміряно 29.08). Гейт стоїть РІВНО
# НА ДЕЛЬТІ: правило на 162 модулях було б міграцією, перевдягненою в інваріант.
#   make mutation-delta-gate BASE=origin/main
mutation-delta-gate: mutation-delta-gate-selftest
	# BASE обовʼязковий, БЕЗ дефолту. `origin/main` як дефолт робив би гейт зеленим за
	# побудовою в лані: там HEAD і origin/main збігаються, дельта порожня, і порожня
	# дельта дає PASS. Гейт, який не має предмета, вимірює власну присутність.
	test -n "$(BASE)"
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/verify_mutation_delta.py --base "$(BASE)"

mutation-delta-gate-selftest:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/verify_mutation_delta.py --selftest

# Karpathy, input-independent baseline: «set all your inputs to zero. This should
# perform worse». Ті самі СВОЇ питання проти вилученого доказового шару. Модель тему
# знає, корпус мовчить: відповідь у цьому стані прийшла з ваг, не з цитат. Вирок — НУЛЬ,
# без порога: поріг був би дозволом відповідати без підстави рідко.
#   make blank-corpus-probe BASE=http://127.0.0.1:8030 TOKEN=...
blank-corpus-probe:
	test -n "$(BASE)"
	test -n "$(TOKEN)"
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/probe_blank_corpus.py --base "$(BASE)" --token "$(TOKEN)"
...
# Момент, коли корпус перестав читатися — і стан МАШИНИ в ту саму мить.
# Чотири пошкодження за три доби, і жодне не має ЧАСУ ПОДІЇ: є лише час випадкового
# виявлення, щоразу за години. Ця проба дешева (два константні запити), тож може бігти
# щохвилини; повноту лишає нічній `corpus-integrity`. При ВІДМОВІ знімає памʼять, своп і
# найбільші процеси — без цього наступна подія знову буде без умови.
#   make corpus-readability [DATABASE=<db>]
# Відновлення обслуговуваного корпусу з бекапа, коли зонд читаності сказав UNREADABLE.
# Дозвіл дає РІВНО його вирок: READABLE — відмова, UNKNOWN — відмова, лише UNREADABLE —
# дія. Пошкоджений файл ЗБЕРІГАЄТЬСЯ поруч як доказ і ніколи не затирається; підписаний
# запис відкату перевіряється тією ж функцією, якою судить служба, а не перевипускається
# наосліп. Без --apply друкує план.
#   make corpus-heal [DATABASE=<db>] [APPLY=1]
corpus-heal:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/heal_corpus_from_backup.py --selftest
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/heal_corpus_from_backup.py \
	  --database "$(or $(DATABASE),$(SERVED_CORPUS))" $(if $(APPLY),--apply)

corpus-readability:
	$(PY) scripts/watch_corpus_readability.py --selftest
	$(PY) scripts/watch_corpus_readability.py --database "$(or $(DATABASE),$(SERVED_CORPUS))"
...
# Ширше питання, ніж у сусіда вище, і поставлене ПЕРШИМ у своєму класі: не «чи цей один
# звіт бачив каталог», а «чи ВСІ звіти доказів описують це дерево, і що перевипускати».
# 02.09.2026 та сама несвіжість вилізла трьома різними голосами — `evidence_provenance`,
# `generated from a different source tree`, `доказ описує стан, якого немає в жодному
# коміті` — і три прогони пішли на те, щоб дізнатись, що причина одна.
# Знаменник доказової поверхні. Виміряно 02.09.2026: прибрати вісь із профілю й прогнати
# `check_answer_axes` — код виходу НУЛЬ, знаменник 16 -> 15, вирок зелений. Зелене від
# скорочення того, що міряють, — найтихіший спосіб оголосити систему готовою.
# ОДНА команда, що доводить готовність. Порядок у ній ВИВЕДЕНИЙ прогонами 02.09.2026, а не
# складений за смаком: виробники перед споживачем, зовнішні гейти після остаточного
# джерела, а після коміту доказу — `release-verify-closure`, бо коміт зсуває HEAD і щойно
# знятий лан стає неприв'язаним.
release-verify:
	PYTHONPATH=apps/api/src $(PY) scripts/run_release_verify.py $(if $(SKIP_EXTERNAL),--skip-external)

release-verify-closure:
	PYTHONPATH=apps/api/src $(PY) scripts/run_release_verify.py --closure-only

release-surface:
	PYTHONPATH=apps/api/src $(PY) scripts/check_release_surface.py
	PYTHONPATH=apps/api/src $(PY) scripts/check_release_surface.py --selftest

evidence-freshness:
	PYTHONPATH=apps/api/src $(PY) scripts/check_evidence_freshness.py
	PYTHONPATH=apps/api/src $(PY) scripts/check_evidence_freshness.py --selftest
...
# Відновлення ОБСЛУГОВУВАНОГО корпусу, коли пошкоджено окремі дерева. Виробник
# з'явився після ДРУГОГО пошкодження за дванадцять годин
# (reports/closure/SQLITE_CORPUS_CORRUPTION_2026-09-08.json): перше відновлювали
# рукою, а рука не є виробником — її довелось би згадувати, а не запускати.
# Без APPLY друкує ПЛАН і не торкається нічого; ціль мусить не існувати.
#   make recover-corpus SOURCE=var/runtime/.../korpus.db TARGET=/шлях/новий.db APPLY=1
recover-corpus:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/recover_sqlite_corpus.py --selftest
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/recover_sqlite_corpus.py \
	  --source "$(SOURCE)" --target "$(TARGET)" $(if $(APPLY),--apply)

# Друга реалізація присуду допуску і вимір РОЗБІЖНОСТЕЙ із виробничою. Соло-власник
# не купує незалежності ІНТЕРЕСУ, а незалежність СЕРЕДОВИЩА виміряна 08.09.2026 і
# закрита на обох дзеркалах. Лишились осі МЕТОДУ і ЧАСУ — це вони:
# протокол зафіксовано хешем ДО прогону, правило виражене вдруге іншими засобами.
# Нуль розбіжностей є твердженням про предмет ЛИШЕ при recall негативного контролю 1.0.
admission-differential:
	PYTHONPATH=apps/api/src:. $(PY) scripts/run_admission_differential.py --selftest
	PYTHONPATH=apps/api/src:. $(PY) scripts/run_admission_differential.py

# Вісім тверджень, які модуль присуду пише ПРО СЕБЕ, перевірені запуском. Не другий
# диференціал: тут ловиться розбіжність між кодом і прозою про нього — клас, на якому
# це дерево вже спотикалось. Композиція перевіряється ВИЧЕРПНО (81 кортеж і всі
# перестановки), осі — на заморожених наборах плюс шість оголошених свідків, бо
# популяція одного класу в корпусі порожня, а `all([])` істинне.
adjudication-properties:
	PYTHONPATH=apps/api/src:. $(PY) scripts/run_adjudication_properties.py --selftest
	PYTHONPATH=apps/api/src:. $(PY) scripts/run_adjudication_properties.py

# Спроба ВІДТВОРИТИ пошкодження бази — на КОПІЇ, з бюджетом, оголошеним до прогону,
...
# `offline-pack` — 503, і `import-corpus` — маніфесту в дереві нема), тоді як дорога
# існувала й працює. `make bootstrap` інжестить ОДИН фікстурний файл, тож свіжий клон
# дає систему з одним демонстраційним документом, і запрошена людина бачить саме її.
# Репетиція наскрізно: reports/closure/CORPUS_TRANSPORT_REHEARSAL.json — 256 документів,
# 256 схвалених, 31464 прольоти, 256 об'єктів перехешовано у ВІДНОВЛЕНІЙ копії.
#   KORPUS_BACKUP_ENCRYPTION_KEY_FILE=... KORPUS_BACKUP_KEY_ID=... make corpus-pack
#   KORPUS_BACKUP_ENCRYPTION_KEY_FILE=... make corpus-install PACK=<файл> INTO=var/runtime/корпус
corpus-pack: backup-sqlite

corpus-install:
	test -n "$(PACK)" || { echo "PACK=<пакет .tar.enc> обовʼязковий" >&2; exit 2; }
	scripts/restore_sqlite.sh "$(PACK)" "$(or $(INTO),var/restored)"

corpus-transport-selftest:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/verify_corpus_transport.py --selftest

corpus-transport-rehearsal:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/verify_corpus_transport.py \
	  $(if $(LIVE),--live "$(LIVE)") $(if $(RESTORED),--restored "$(RESTORED)")
...
# Owner оголосив 03.09.2026, що зовнішнього незалежного оцінювача в проєкті немає й він
# не є доступним ресурсом. Заміна законна саме тому, що не є послабленням: зникла умова,
# якої не можна задовольнити наявними ресурсами, а кожна, яку МОЖНА, лишилась блокуючою.
# Цей гейт існує, щоб друге твердження було перевіреним, а не обіцяним.
# РОЗДІЛЕНО НАВМИСНО. Перша редакція поставила повний вимір у `validate` — і створила
# кільце: `verify-clean-clone` женe `validate` у свіжому клоні, де немає ні звіту про
# відтворення, ні засвідчення верифікатора, ні живих процесів. Гейт вимагав результату
# прогону, всередині якого сам і був.
#
# Здатність ЛОВИТИ самопідпис не залежить від рантайму, тож самоперевірка лишається в
# `validate` і бігає всюди, включно з чистим клоном. Сам ВИМІР питає про стан кандидата
# й розгортання, тому живе в `check-deployment`.
assurance-model-selftest:
	$(PY) scripts/verify_assurance_model.py --selftest

# Зелений локальний гейт не є моделлю зеленої CI. Виміряно 03.09.2026: замінник SI-7
# читав `var/ci-mirror.json`, якого не виробляв ніхто; перший вимір дав 61/73 команд
# лану в CI, з них одна справжня діра (транспортний контракт) і решта — оформлення
# тотожності (перенаправлення, `$(if …)`, `--selftest` через selftest-coverage).
ci-mirror:
...
# Заморозка кандидата: тотожність стає незмінною, знаменник гейтів — сталим. Ціль не
# створює протоколу — вона вхід у наявну машину станів релізу.
release-freeze:
...
# Підписати ВІДКАТ журналу, що стався через законне відновлення з бекапа. Доти єдиною
# дорогою назад було СТЕРТИ якір — тобто знищити доказ, заради якого він існує. Запис
# нічого не оголошує: перевіряються автентичність маніфеста бекапа, належність голови
# ланцюгу і те, що якір справді попереду. Без --apply друкує план.
#   make attest-restore DATABASE=<db> ANCHOR=<файл> BACKUP=<tar.enc> \
#     BACKUP_KEY=<файл> AUDIT_KEY=<файл> [APPLY=1]
attest-restore:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/attest_restore.py --selftest
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/attest_restore.py \
	  --database "$(DATABASE)" --anchor "$(ANCHOR)" --backup "$(BACKUP)" \
	  --backup-key-file "$(BACKUP_KEY)" --audit-key-file "$(AUDIT_KEY)" $(if $(APPLY),--apply)
...
# Набір на PostgreSQL — у нічному лані, а не «обов'язкова робота CI» на словах.
# 01.09.2026 прогін на СУБД розгортання дав чотири вади, яких SQLite показати не
...
check-deployment: serving-freshness serving-readiness edge-config-parity assurance-model runtime-corpus-audit corpus-integrity audit-verify deployment-debt evidence-stores pilot-posture

# Свіжість коду і здатність обслуговувати — РІЗНІ властивості, і друга не вимірювалась
# ніде. Виміряно 06.09.2026: публічний сервіс ніс поточну ревізію і три доби віддавав
# 503 КОЖНОМУ запиту, а лан розгортання лишався зелений, бо питав лише про ревізію.
# Чи кожен артефакт доказу знає, ПРО ЩО він, ЗА ЯКИХ УМОВ і хто його читає. Гейт не
# питає, чи вирок правильний — він питає, чи артефакт здатен бути неправильним у спосіб,
# який хтось помітить. Пʼять осей, кожна з власним експериментом; `--independence` —
# негативний контроль самої таксономії, здатний зняти вісь.
evidence-epistemics:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/check_evidence_epistemics.py --independence

evidence-epistemics-selftest:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/check_evidence_epistemics.py --selftest

serving-readiness:
	$(PY) scripts/check_serving_readiness.py

serving-readiness-selftest:
	$(PY) scripts/check_serving_readiness.py --selftest
...
# `owner-packet-check` стоїть ТУТ, не у `validate`: пакет власника несе дайджест
...
# ДВА середовища, два замки, і жодне не проходить перевірок іншого. Локально
# стоять обидва замки (dev-інструменти є), але інтерпретатор не продакшенний;
...
# Профіль РОЗРОБКИ. Пише `exact_environment-development-gate.json` і канонічного
# артефакта не торкається: доти обидві цілі писали один файл, і ця — та, що стоїть
# у `validate`, — знищувала доказ образу щоразу, коли біг лан.
...
production-exact-environment-image:
	docker build -f apps/api/Dockerfile -t korpus-api:exact-env .
	docker run --rm --user root -v "$(CURDIR):/src" -w /src \
	  -e PYTHONPATH=/src/apps/api/src:/src/scripts \
	  korpus-api:exact-env python scripts/run_exact_environment_gate.py --profile runtime

production-hard-predicates:
	PYTHONPATH=apps/api/src:scripts $(PY) scripts/verify_production_hard_predicates.py

# Репродукція кандидата з ВІДДАЛЕНОГО джерела зі свіжими залежностями. Артефакт
# `reports/closure/CLEAN_ROOM_REPRODUCTION.json` доти не мав виробника в дереві: його
...
evidence-refresh:
	...
	# Пакет власника — теж доказова поверхня, і `current-truth-verify` гейтує його
	# `source_bound`. Доти єдиним виробником була рука, тож після кожної правки джерела
	# лан спинявся на документі, який ніхто не оновлював. Виробник стоїть ПЕРЕД
	# перевіркою, як і решта виробників у цьому рецепті.
	$(MAKE) owner-packet PY=$(PY)
	$(MAKE) current-truth-verify PY=$(PY) OUT=reports/CURRENT_TRUTH_VERIFICATION.json
