...
- Canonical release: `v6.18.0` ACT-015 supply-chain container-image evidence closure.
- Production authorization: `false` until fresh `v6.18.0` evidence completes all required production gates.
...
- `v6.18.0` binds API/Web container scan success into the signed supply-chain evidence domain, requires exact artifact-set equality, and rejects stale scanner markers from another commit. Application runtime semantics are unchanged.
...
Interpretation: `v6.18.0` is the canonical engineering candidate only after fresh regression, coverage, mutation, browser E2E and assurance bind to its immutable source identity. Prior PASS artifacts are historical baseline only.
