...
_FALLBACKS = frozenset(
    {
        ...
        "answer_calibration_mismatch",
        "leaf_not_admitted",
        "semantic_retrieval_unavailable",
        "state_out_of_support",
        "support_bound",
        "shadow_mode",
        "other",
    }
...
def _fallback_label(reason: str | None) -> str:
    ...
    if reason.startswith(
        ("state_below_support:", "state_above_support:", "unsupported_non_numeric_feature:")
    ):
...
class PECMetrics:
    ...
    def __init__(self, registry: CollectorRegistry) -> None:
        self.actions = Counter(
            "korpus_pec_actions_total", "PEC effective actions.", ["action"], registry=registry
        )
        self.fallbacks = Counter(
            "korpus_pec_fallbacks_total",
            "PEC bounded fallback reasons.",
            ["reason"],
            registry=registry,
        )
        self.planner = Counter(
            "korpus_pec_planner_total",
            "Planner execution under PEC.",
            ["executed"],
            registry=registry,
        )
        self.semantic = Counter(
            "korpus_pec_semantic_total",
            "Semantic retrieval execution under PEC.",
            ["executed"],
            registry=registry,
        )
        self.first_pass = Counter(
            "korpus_pec_first_pass_total",
            "First-pass evidence sufficiency.",
            ["sufficient"],
            registry=registry,
        )
        self.out_of_support = Counter(
            "korpus_pec_out_of_support_total",
            "PEC states outside admitted support.",
            registry=registry,
        )
        self.latency = Histogram(
            "korpus_pec_retrieval_duration_seconds",
            "PEC retrieval orchestration latency.",
            ["action"],
