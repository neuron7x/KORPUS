...
def amount_minor(value: Any, maximum_minor: int) -> int | None:
    ...
    if isinstance(value, bool):
        raise ValueError("LiqPay amount is not numeric")
    try:
        major = Decimal(str(value))
    except (DecimalException, ValueError) as exc:
        raise ValueError("LiqPay amount is not numeric") from exc
    if not major.is_finite():
        raise ValueError("LiqPay amount is not finite")
    if major <= 0:
        raise ValueError("LiqPay amount is not positive")
    if major > Decimal(maximum_minor) / 100:
        raise ValueError("LiqPay amount exceeds the sellable-plan domain")
    try:
        amount = major * 100
    except DecimalException as exc:
        raise ValueError("LiqPay amount is not representable") from exc
