...
def _provider_decimal(value: object) -> object:
    ...
    if not value.isascii() or not value.isdecimal():
        raise ValueError("provider count must be an ASCII decimal integer")
    return int(value)
...
def count(value: object, minimum: int, label: str, *, allow_digit_string: bool = False) -> int:
    ...
    if not strict_int(parsed) or parsed < minimum:
        raise ValueError(
            f"{label} must be {'positive integer' if minimum == 1 else f'an integer >= {minimum}'}"
        )
    return parsed
...
def embedding_limits(
    dimensions: object, attempts: object, maximum_bytes: object, seconds: object
) -> tuple[int, int, int, float]:
    return (
        count(dimensions, 8, "dimensions"),
        count(attempts, 1, "max_attempts"),
        count(maximum_bytes, 1024, "max_response_bytes"),
