...
class SqlCorpusSnapshotReader:
    ...
    def __init__(self, repository: SqlRepository) -> None:
        ...
        # Читач чіпляється до репозиторію ЗБОКУ: `SqlRepository` про знімок не знає
        # й знати не мусить — знімок ОПИСУЄ читання, а не є його частиною. Оголошений
        # атрибут зробив би залежність двобічною, тож тут вона названа явно.
        repository.corpus_snapshot_reader = self  # type: ignore[attr-defined]
    ...
    def capture(
        ...
        with self.engine.begin() as connection:
            self.repository._apply_postgres_identity(connection, identity)  # noqa: SLF001
