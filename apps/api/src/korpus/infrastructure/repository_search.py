...
def search_retrievable_spans(
    ...
    with repository.engine.begin() as connection:
        ...
        span_ids = repository._candidate_span_ids(identity, corpora, as_of, query, candidate_limit * 4, connection)
        if not span_ids:
            return []
        statement = retrieval_queries.retrievable_projection(identity, corpora, as_of).where(spans.c.id.in_(span_ids))
...
def _alias_document_ids(
    ...
    for document_id, aliases in sorted((approved_aliases or {}).items()):
        alias_terms = {
            ...
            for value, _ in candidate_terms(alias)
        }
...
def search_contextual_retrievable_spans(
    ...
    *,
    ...
    if candidate_limit < 1:
        raise ValueError("candidate_limit must be positive")
    baseline = search_retrievable_spans(repository, identity, corpus_ids, as_of, query, candidate_limit)
    corpora = corpus_ids.intersection(identity.corpora)
    ...
    statement = retrieval_queries.retrievable_projection(identity, corpora, as_of).where(or_(*predicates))
    if baseline_ids:
