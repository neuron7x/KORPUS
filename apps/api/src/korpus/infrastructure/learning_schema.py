...
learning_course_versions = Table(
    ...
    Column("created_at", DateTime(timezone=True), nullable=False),
    UniqueConstraint("course_id", "revision", name="uq_learning_course_version_revision"),
)
...
learning_modules = Table(
    ...
    UniqueConstraint(
        "course_version_id",
        "ordinal",
        name="uq_learning_module_ordinal",
    ),
)
...
learning_lessons = Table(
    ...
    UniqueConstraint(
        ...
        "module_id",
        "ordinal",
        name="uq_learning_lesson_ordinal",
    ),
)
...
learning_objectives = Table(
    ...
    UniqueConstraint(
        ...
        "lesson_id",
        "ordinal",
        name="uq_learning_objective_ordinal",
    ),
)
...
learning_source_bindings = Table(
    ...
    Column("version_id", String(36), ForeignKey("document_versions.id"), nullable=False),
    ForeignKeyConstraint(
        ["course_version_id", "lesson_id"],
        ["learning_lessons.course_version_id", "learning_lessons.id"],
        ondelete="CASCADE",
        name="fk_learning_source_lesson",
    ),
)
...
learning_source_binding_spans = Table(
    ...
    Column("span_id", String(36), ForeignKey("evidence_spans.id"), primary_key=True),
    ForeignKeyConstraint(
        ...
        ondelete="CASCADE",
        name="fk_learning_source_span_binding",
    ),
)
...
learning_lesson_blocks = Table(
    ...
    UniqueConstraint(
        ...
        "lesson_id",
        "ordinal",
        name="uq_learning_block_ordinal",
    ),
)
...
learning_prerequisites = Table(
    ...
    CheckConstraint(
        ...
        name="ck_learning_no_self_prerequisite",
    ),
)
...
learning_publications = Table(
    ...
    CheckConstraint(
        ...
        name="ck_learning_published_reviewed",
    ),
)
