"""Межа особистості RLS, яку застосунковий логін не може підробити."""
...
class RlsBoundSqlRepository(SqlRepository):
    """SqlRepository, чий контекст RLS записує ОКРЕМИЙ логін.

    Політики читали `current_setting('korpus.*')`, а ці ж значення писав сам
    застосунковий логін через `set_config`. RLS вірив тому, кого мав обмежувати.

    Виміряно ЗАПУСКОМ 01.09.2026 на цьому дереві, а не взято з документа: логін
    `korpus_app` підняв собі `korpus.clearance` 0 → 3 і `korpus.roles` user → admin
    і ПОБАЧИВ та ЗАПИСАВ документ грифу `restricted`, якого чесна особистість не
    бачила. Отже RLS не була незалежною межею — вона була угодою застосунку з собою.

    Тепер claim'и живуть у `public.korpus_rls_context`, до якої застосунковий логін
    не має жодного доступу, а кладе їх туди брокер: окремий логін через
    `SECURITY DEFINER`-функцію, яка сама звіряє, що прив'язка стосується ТОГО
    САМОГО backend'а й ТІЄЇ САМОЇ транзакції, що їх назвав викликач.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if database_url.startswith("postgresql"):
            self._validate_authz_url(database_url, authz_database_url)
        super().__init__(
            database_url,
            audit_hmac_key,
            *args,
        ...
        if database_url.startswith("postgresql"):
            self.authz_engine = create_engine(
                ...
                future=True,
                pool_pre_ping=True,
                # NullPool: брокерське з'єднання живе рівно один виклик. Пул тримав би
                # відкриті сесії привілейованішого логіна між запитами.
                poolclass=NullPool,
                connect_args={
    ...
    @staticmethod
    def _validate_authz_url(database_url: str, authz_database_url: str | None) -> None:
    ...
    def _apply_postgres_identity(self, connection: Connection, identity: Identity) -> None:
        ...
        parameters = {
            ...
            # `korpus.subject` ставив окремий виклик в `ingestion_jobs.py`, і брокер
            # GitHub-лінії його не носив: політика черги порівнювала б власника
            # роботи з порожнім рядком.
