...
from korpus.infrastructure.audit_anchor import AuditAnchorStore, FileAuditAnchorStore, HttpAuditAnchorStore
...
def create_audit_anchor(settings: Settings, key: bytes) -> AuditAnchorStore:
    if settings.audit_anchor_mode == "http":
        return HttpAuditAnchorStore(
            settings.audit_anchor_url or "", key,
    ...
    if settings.audit_anchor_mode == "gcs":
        return GcsAuditAnchorStore(
            settings.gcs_audit_bucket or "", key, prefix=settings.gcs_audit_prefix,
        )
