...
def _matches(actual: object, condition: RuleCondition) -> bool:
    expected=condition.value; op=condition.operator
    if op=="eq": return actual==expected
    if op=="ne": return actual!=expected
    if isinstance(actual,bool) or not isinstance(actual,(int,float)): return False
    if isinstance(expected,bool) or not isinstance(expected,(int,float)): return False
    if not finite_number(actual) or not finite_number(expected): return False
    return {"lt":actual<expected,"le":actual<=expected,"gt":actual>expected,"ge":actual>=expected}.get(op,False)
...
def _candidates(rows: list[TrainingRow]) -> Iterable[RuleCondition]:
    for feature in FEATURE_NAMES:
        ...
        if all(isinstance(v,bool) for v in unique):
            yield RuleCondition(feature=feature,operator="eq",value=True); continue
        if all(isinstance(v,(int,float)) and not isinstance(v,bool) for v in unique):
            ...
            numeric=sorted(float(v) for v in unique)
            if len(numeric)>24:
                step=max(1,len(numeric)//24); numeric=numeric[::step]
            for left,right in zip(numeric,numeric[1:]):
                if left!=right: yield RuleCondition(feature=feature,operator="le",value=(left+right)/2)
            continue
        for value in unique[:16]:
            if isinstance(value,str): yield RuleCondition(feature=feature,operator="eq",value=value)


def _split(rows:list[TrainingRow],condition:RuleCondition)->tuple[list[TrainingRow],list[TrainingRow]]:
...
def _best_split(rows:list[TrainingRow],min_leaf:int)->tuple[RuleCondition,list[TrainingRow],list[TrainingRow]]|None:
    base=_errors(rows); best=None; best_key=None
    for condition in _candidates(rows):
        ...
        if gain>0 and (best_key is None or key<best_key): best=(condition,left,right); best_key=key
    return best


def train_tree(rows:Iterable[TrainingRow],*,max_depth:int,min_leaf:int)->TreeModel:
    data=list(rows)
    if not data: raise ValueError("PEC training requires rows")
    ...
    def walk(node:list[TrainingRow],depth:int,path:tuple[RuleCondition,...])->None:
        split=None if depth>=max_depth or len(node)<2*min_leaf or _errors(node)==0 else _best_split(node,min_leaf)
        if split is None:
            leaves.append(TreeLeaf(f"leaf-{len(leaves):03d}",path,_majority(node),len(node))); return
        condition,left,right=split
        walk(left,depth+1,(*path,condition))
        inverse={"le":"gt","lt":"ge","ge":"lt","gt":"le","eq":"ne","ne":"eq"}[condition.operator]
        walk(right,depth+1,(*path,RuleCondition(feature=condition.feature,operator=inverse,value=condition.value)))
    walk(data,0,())
