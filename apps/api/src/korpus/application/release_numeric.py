...
def preflight_report_pass(name: str, report: Mapping[str, Any], policy: Mapping[str, Any]) -> bool:
    if name == "backend":
        return (
            nonnegative_count(report.get("failed"), allow_digit_string=True) == 0
            and nonnegative_count(report.get("errors"), allow_digit_string=True) == 0
        )
    if name == "coverage":
        ...
        if statement is None or branch is None or line_floor is None or branch_floor is None:
            return False
        return statement >= 100 * line_floor and branch >= 100 * branch_floor
    return report.get("status") == "PASS"


def mutation_checks(
    report: Mapping[str, Any], source_bound: bool
) -> tuple[dict[str, bool], int, int, int]:
    total = nonnegative_count(report.get("mutants"))
    valid = nonnegative_count(report.get("valid_mutants"))
    killed = nonnegative_count(report.get("killed"))
    formed = (
        total is not None
    ...
    checks = {
        ...
        "counts_well_formed": formed,
        "catalogue_nonempty": formed and total > 0,
        "all_mutants_valid": formed
        and valid == total
        and not report.get("invalid")
