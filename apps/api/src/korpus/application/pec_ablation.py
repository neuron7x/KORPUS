...
def regressions(
    ...
    safety = sum(
        ...
        not _bool(baseline[q], "answer_error") and _bool(candidate[q], "answer_error") for q in ids
    )
    quality = sum(
        _bool(baseline[q], "quality_ok") and not _bool(candidate[q], "quality_ok") for q in ids
    )
...
def resource_comparisons(
    ...
    for field in RESOURCE_FIELDS:
        ...
        supported = (
            comparison.informative_pairs >= minimum_pairs and comparison.lower_win_probability > 0.5
        )
        ...
        resources[field] = {
            ...
            "win_probability_interval_95": [
                comparison.lower_win_probability,
                comparison.upper_win_probability,
            ],
