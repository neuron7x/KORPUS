...
def evaluate_human_judgments(rows: Iterable[Mapping[str, object]], *, expected_case_ids: Iterable[str], binding: RevisionBinding) -> JudgmentVerdict:
    ...
    return JudgmentVerdict(not failures and bool(materialized), tuple(sorted(set(failures))), len(materialized))
