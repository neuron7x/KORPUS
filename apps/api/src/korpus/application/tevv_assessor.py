...
def assessor_identity_valid(evidence: dict[str, Any]) -> bool:
    ...
    if not isinstance(assessor, dict):
        return False
    return (
        isinstance(assessor.get("organization"), str)
        and bool(assessor.get("organization"))
        and isinstance(assessor.get("assessor_id"), str)
        and bool(assessor.get("assessor_id"))
