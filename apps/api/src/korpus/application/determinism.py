...
def run_json(
    command: list[str], env: dict[str, str], root: Path, timeout: int
...
def run_seed(seed: int, junit: Path, root: Path, tests: tuple[str, ...]) -> dict[str, object]:
    ...
    counts = (
        junit_contract(junit)
        if junit.is_file()
        else {"tests": 0, "failures": 1, "errors": 1, "skipped": 0, "outcome_sha256": ""}
    )
    ...
    return {
        ...
        "semantic_replay_sha256": str(replay.get("sha256", "")),
        **counts,
    }
...
def failures(runs: list[dict[str, object]], policy: dict[str, object]) -> list[str]:
    ...
    failed = any(
        r["exit_code"] or r["replay_exit_code"] or r["failures"] or r["errors"] for r in runs
    )
    ...
    checks = (
        ...
        (
            len(replays) == 1 and "" not in replays,
            "semantic replay digest differs across hash seeds",
        ),
    )
