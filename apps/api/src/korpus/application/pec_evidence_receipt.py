...
def canonical_receipt(payload: Mapping[str, object], *, release: str, source_digest: str, collection_digest: str) -> dict[str, object]:
    ...
    if any(len(value) != 64 or any(ch not in "0123456789abcdef" for ch in value) for value in normalized.values()):
        raise ValueError("artifact digests must be lowercase sha256")
    body = {"release": release, "source_digest": source_digest, "collection_digest": collection_digest, "artifacts": normalized}
