...
def counters_within_total(total: object, *counts: object) -> bool:
    return (
        strict_int(total) and total >= 0 and all(strict_int(v) and 0 <= v <= total for v in counts)
    )


def validate_embedding_coverage(
    model_id: str, dimensions: object, total: object, active: object, other: object, stale: object
) -> None:
    ...
    if not counters_within_total(total, active, other, stale):
        raise ValueError(
            "embedding coverage counters must be non-negative integers bounded by total spans"
        )
