...
def check_grid(policy: AdaptationPolicy) -> dict[str, object]:
    ...
    for samples, error, contradiction, overload, latency, recall, healthy_count, last_change in grid:
        state = AdaptationState(base, last_change_sequence=last_change,
        ...
        checks = [
            ...
            (policy.min_candidate_budget <= left.proposed.candidate_budget <= policy.max_candidate_budget,
             "candidate budget escaped bounds"),
            (policy.min_timeout_ms <= left.proposed.retrieval_timeout_ms <= policy.max_timeout_ms,
             "timeout escaped bounds"),
        ]
        unsafe = error > policy.high_error_rate or contradiction > policy.high_contradiction_rate
        checks.append((samples < policy.min_samples or not unsafe or
                       left.action is AdaptationAction.TIGHTEN_SAFETY,
