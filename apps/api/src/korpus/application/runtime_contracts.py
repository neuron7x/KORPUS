...
def admission_parameters(
    capacity: object, timeout: object, per_subject: object
    ...
    if per_subject is not None and (
        not strict_int(per_subject) or not 1 <= per_subject <= capacity
    ):
...
def circuit_parameters(threshold: object, timeout: object) -> tuple[int, float]:
    ...
    if not finite_number(timeout) or timeout <= 0:
        raise ValueError(
            "invalid circuit breaker recovery_timeout_seconds: must be finite and positive"
        )
