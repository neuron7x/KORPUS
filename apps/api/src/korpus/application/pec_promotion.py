...
REQUIRED_RECEIPTS = ("dataset_audit", "counterfactual_replay", "oracle", "training", "controller_verify", "ablation", "metamorphic", "mutation", "regression", "current_truth")
...
def promotion_errors(profile: ControllerProfile, statuses: Mapping[str, str]) -> list[str]:
    ...
    errors: list[str] = []
    if missing: errors.append(f"missing_receipts:{','.join(missing)}")
    if nonpass: errors.append(f"nonpass_receipts:{','.join(nonpass)}")
    if profile.admission_status != "PASS": errors.append("profile_not_admitted")
    return errors
