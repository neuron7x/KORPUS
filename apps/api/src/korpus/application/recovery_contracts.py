...
def recovery_numeric_problem(
    ...
    invalid = [
        ...
        if nonnegative_count(provenance.get(f), allow_digit_string=True) is None
    ]
    if invalid:
...
def recovery_scale_counts(provenance: Mapping[str, Any]) -> tuple[int, int]:
    return nonnegative_count(
        provenance.get("document_rows"), allow_digit_string=True
    ) or 0, nonnegative_count(provenance.get("plaintext_bytes"), allow_digit_string=True) or 0
