...
def _semantic_candidates(
    ...
    if missing:
        candidates.extend(
            repository.get_retrievable_spans_by_ids(identity, corpus_ids, as_of, missing)
        )
    ...
    for scored in components:
        ...
        output.append(
            RetrievedEvidence(
                ...
                query_coverage=scored.query_coverage,
                lexical_score=scored.lexical_score,
                semantic_score=scored.semantic_score,
                character_score=scored.character_score,
                authority_bonus=scored.authority_score,
            )
        )
    output.sort(
        key=lambda item: (
            -authority_priors[item.version.authority],
            -item.score,
            -item.query_coverage,
...
def execute_hybrid_search(
    *,
    ...
    return execute_hybrid_search_impl(
        ...
        contextual_projection_enabled=contextual_projection_enabled,
        ...
        materialize_evidence=_materialize_evidence,
        deadline_error=ExecutionDeadlineExceeded,
    )
