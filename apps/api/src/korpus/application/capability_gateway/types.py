...
class CapabilitySpec(BaseModel):
    ...
    def validate_execution_invariants(self) -> CapabilitySpec:
        ...
        if self.authorization.requires_explicit_effect_authorization and not effectful:
            raise ValueError(
                "explicit effect authorization is valid only for effectful capabilities"
            )
        ...
        if (
            ...
            and not self.idempotency.provider_key_forwarding
        ):
            raise ValueError(
                "effectful multiple attempts require provider idempotency-key forwarding proof"
            )
        if (
            self.evidence.profile is not EvidenceProfile.NONE
            and not self.evidence.bind_output_digest
        ):
