...
def effect_safety_graph_errors(
    ...
    for spec in ordered:
        ...
        if target_id is None or target_version is None:
            errors.append(f"{source_key}: compensation capability identity is incomplete")
            continue
        ...
        if target is None:
            errors.append(f"{source_key}: compensation capability is not registered: {target_key}")
            continue
        ...
        if target.effect_class not in _EFFECTFUL:
            errors.append(f"{source_key}: compensation capability must be effectful: {target_key}")
            continue
