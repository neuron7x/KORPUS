    ...
    *,
    ...
    if updated.reconciliation_disposition is not None:
        raise InvalidEffectTransition(
            "ordinary effect transition cannot set reconciliation disposition"
        )
    return updated
...
def attest_reconciled_effect(
    ...
    *,
    ...
    if updated.provider_reference != provider_reference:
        raise InvalidEffectTransition(
            "reconciliation provider reference does not match observation"
        )
    return updated
    ...
    *,
    ...
    if reservation.created:
        ...
        if record.invocation_id != invocation_id:
            raise InvalidEffectReservation(
                "new effect reservation invocation binding is inconsistent"
            )
        if record.provider_reference is not None or record.reconciliation_disposition is not None:
            raise InvalidEffectReservation(
                "new effect reservation contains terminal provider state"
            )
    return reservation
...
def assert_effect_transition(current: EffectState, target: EffectState) -> None:
    if target not in _ALLOWED_TRANSITIONS[current]:
        raise InvalidEffectTransition(
            f"forbidden effect transition: {current.value} -> {target.value}"
        )
