from korpus.application.pec_research import (
    conditional_risk_report, observed_information_gain, production_judgment_validity,
    replay_priority_enrichment, simultaneous_hoeffding_upper,
)


def test_simultaneous_bound_is_more_conservative_with_more_strata() -> None:
    assert simultaneous_hoeffding_upper(0, 100, 0.05, 4) > simultaneous_hoeffding_upper(0, 100, 0.05, 1)


def test_conditional_risk_underpowered_stratum_falls_back() -> None:
    rows = (
        [{"risk": "underpowered", "error": False} for _ in range(3)]
        + [{"risk": "sufficient_but_unsafe", "error": True} for _ in range(30)]
    )
    report = conditional_risk_report(rows, stratum_key="risk", error_key="error", risk_limit=0.2, delta=0.05, minimum_samples=30)
...
def test_replay_priority_enriches_explicit_failures() -> None:
    ...
    for i in range(8): rows[i]["accepted_answer_error"] = True
...
def test_production_judgment_requires_bound_provenance() -> None:
    report = production_judgment_validity([{"id":"q","production_judged":True,"judgment_provenance_sha256":"x"*64,"adjudication_protocol":"v1"}])
