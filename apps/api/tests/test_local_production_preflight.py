...
def _root(tmp_path: Path) -> Path:
    ...
    for name, filename in REPORT_NAMES.items():
        ...
        if name == "coverage":
            payload.update(
                {"statement_coverage_percent": 95.0, "branch_coverage_percent": 90.0}
            )
...
def test_local_success_never_claims_production_authorization(tmp_path: Path, monkeypatch) -> None:
    root = _root(tmp_path)
    monkeypatch.setattr("scripts.run_local_production_preflight.release_tag", lambda _root=None: RELEASE)
