...
def test_review_credential_is_provisioned_and_mounted_only_where_needed() -> None:
    ...
    review_template = api["environment"]["KORPUS_REVIEW_DATABASE_URL_TEMPLATE"]
    assert "korpus_review:{password}" in review_template
...
def test_entrypoint_builds_review_url_without_leaving_password_intermediates() -> None:
    ...
    assert "build_database_url" in entrypoint
    assert "KORPUS_REVIEW_DATABASE_URL" in entrypoint
