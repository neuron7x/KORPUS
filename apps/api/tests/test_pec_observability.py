...
from korpus.application.pec_metrics_context import emit_pec_observation, reset_pec_observer, set_pec_observer
...
def _trace(reason: str | None = None) -> ControllerTrace:
    return ControllerTrace(
        profile_id="p", profile_digest="a" * 64, state_fingerprint="b" * 64,
        predicted_action=RetrievalAction.PLAN_QUERY_VARIANTS,
        effective_action=RetrievalAction.PLAN_QUERY_VARIANTS,
        rule_id="r", leaf_id="l", admitted=True, fallback_reason=reason,
        shadow_mode=False, first_pass_sufficient=False, planner_executed=True,
    )
