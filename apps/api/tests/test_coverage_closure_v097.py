...
def test_hosted_mapping_and_binding_cover_missing_and_invalid() -> None:
    ...
    bad = binding_payload(); bad["release"] = "v0.9.6"
...
def test_hosted_audit_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(hosted, "extract_audit_trace", lambda rows, b: SimpleNamespace(event_ids=(), sha256="empty"))
    assert hosted._audit({"audit_rows": []}, binding) == (False, "empty", ["audit_trace:empty"])
    monkeypatch.setattr(hosted, "extract_audit_trace", lambda rows, b: SimpleNamespace(event_ids=("e1",), sha256="ok"))
    assert hosted._audit({"audit_rows": [{}]}, binding) == (True, "ok", [])
    monkeypatch.setattr(hosted, "extract_audit_trace", lambda rows, b: (_ for _ in ()).throw(ValueError("bad")))
...
def test_hosted_training_judgment_and_receipt_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(hosted, "validate_training_lineage", lambda *a, **k: SimpleNamespace(valid=True, failures=()))
    ...
    monkeypatch.setattr(hosted, "evaluate_human_judgments", lambda *a, **k: SimpleNamespace(admissible=True, judgments=2, failures=()))
    assert hosted._judgments({"expected_case_ids": ["a"], "judgments": [{}]}, binding) == (True, 2, [])

    assert hosted._hosted({}, release="v0.9.7", source_digest=SHA)[1] == ["hosted_evidence:missing"]
    receipt = {"provider": "github-actions", "run_id": "1", "workflow": "w", "release": "v0.9.7", "source_digest": SHA, "local_self_attested": False}
    assert hosted._hosted({"hosted_receipt": receipt}, release="v0.9.7", source_digest=SHA) == (True, [])
...
def test_hosted_authority_canary_and_top_level(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    authority, got_binding, failures = hosted._authority_gate(
        evidence, release="v0.9.7", source_digest=SHA,
        attestation_verified=True, trusted_signer=True, signer_fingerprint="fp",
    )
    ...
    canary = hosted._canary_gate(evidence, authority=authority, binding=binding, release="v0.9.7", source_digest=SHA)
    assert canary["status"] == "PASS"

    result = hosted.evaluate_pec_production_evidence(
        evidence, release="v0.9.7", source_digest=SHA,
        attestation_verified=True, trusted_signer=True,
    )
    assert result["status"] == "PASS"

    missing = hosted._canary_gate({}, authority={"status": "FAIL"}, binding=None, release="v0.9.7", source_digest=SHA)
    assert missing["status"] == "FAIL"


def _meta_row(pair: str, variant: str, **extra: object) -> dict[str, object]:
    row: dict[str, object] = {
        "pair_id": pair, "variant": variant,
        "source_digest": SHA, "corpus_release_id": "1" * 16,
        "evaluation_protocol_sha256": "b" * 64, "answer_calibration_id": "cal-1",
    }
    row.update(extra)
    return row
...
def test_metamorphic_eval_binding_and_issue_failures(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    report = meta.evaluate_metamorphic_pairs([_meta_row("p1", "base"), transformed], minimum_pairs=1)
    ...
    incomplete_binding = _meta_row("p2", "base"); incomplete_binding["answer_calibration_id"] = ""
    transformed2 = dict(incomplete_binding); transformed2["variant"] = "transformed"
    monkeypatch.setattr(meta, "metamorphic_issues", lambda base, row: [])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (json.dumps({"release": "v0.9.7", "source_digest": SHA, "status": "PASS"}), ".json", "SUPPORTED"),
        (json.dumps({"release": "v0.9.7", "source_digest": SHA, "status": "FAIL"}), ".json", "REFUTED_BY_EVIDENCE"),
    ],
)
def test_release_claim_status_branches(tmp_path: Path, content: str | None, suffix: str, expected: str) -> None:
...
class _SemanticDelegate(_Delegate):
    def semantic_available(self) -> bool:
        return True
    def search_with_semantic(self, *args: object, semantic_enabled: bool, **kwargs: object) -> list[object]:
        self.calls += 1
        return []
...
def test_pec_cache_semantic_availability_and_fallback() -> None:
    ...
    assert cached.search_with_semantic(_identity(), "q", frozenset({"c"}), date(2026, 8, 23), semantic_enabled=True) == []
...
def test_gcp_id_token_cache_invalid_payload_marker_and_close() -> None:
    ...
    invalid = MetadataIdentityProvider(client=httpx.Client(transport=httpx.MockTransport(lambda r: httpx.Response(200, headers={"Metadata-Flavor": "Google"}, text="bad"))))
    with pytest.raises(MetadataIdentityError, match="ID-token"):
        invalid.id_token("https://service.example")

    hostile = MetadataIdentityProvider(client=httpx.Client(transport=httpx.MockTransport(lambda r: httpx.Response(200, headers={"Metadata-Flavor": "Hostile"}, json={"access_token": "x", "expires_in": 10, "token_type": "Bearer"}))))
...
def test_gcs_json_upload_invalid_metadata_and_name_mismatch() -> None:
    with pytest.raises(ObjectStoreUnavailable, match="invalid metadata"):
        _gcs_client(lambda r: httpx.Response(200, content=b"bad")).upload_create_only("objects/x", b"x")
    with pytest.raises(ObjectStoreUnavailable, match="does not identify"):
        _gcs_client(lambda r: httpx.Response(200, json={"name": "other"})).upload_create_only("objects/x", b"x")
...
def test_gcs_json_list_paginates_and_respects_max_results() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if calls == 1:
            ...
            return httpx.Response(200, json={"items": [{"name": "objects/a"}, {"name": ""}], "nextPageToken": "next"})
    ...
    def max_handler(request: httpx.Request) -> httpx.Response:
        nonlocal max_calls
        max_calls += 1
        return httpx.Response(200, json={"items": [{"name": "objects/a"}, {"name": "objects/b"}], "nextPageToken": "x"})
...
def test_gcs_object_store_validation_size_exists_list_and_health(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="size"):
        s.put(b"four", "0" * 64, "x")
    p = tmp_path / "large"; p.write_bytes(b"four")
    with pytest.raises(ValueError, match="size"):
        s.put_path(p, "0" * 64, "x")

    b = _MemoryBackend(); s2 = _store2(b)
    ...
    b.retention = 100; b.bucket = "other"
    assert s2.healthcheck() is False
    b.bucket = "korpus-objects"; b.fail_bucket = True
    assert s2.healthcheck() is False
    s2.close(); assert b.closed


def test_gcs_object_store_read_and_verify_failure_modes() -> None:
    b = _MemoryBackend(); s = _store2(b, max_object_bytes=3)
    digest = __import__("hashlib").sha256(b"abc").hexdigest(); key = s._key(digest)
...
from korpus.application.controller_profile import (
    ControllerLeaf, ControllerProfile, ControllerRule, FeatureRange, RuleCondition,
)
...
from korpus.application.predictive_evidence_control import (
    PredictiveEvidenceController, RetrievalAction, _condition_matches, _support_failure,
)


def _controller_profile(
    *, admission_status: str = "PASS", rules: tuple[ControllerRule, ...] | None = None,
    minimum_leaf_samples: int = 1, risk_limit: float = 0.1,
) -> ControllerProfile:
    if rules is None:
        rules = (
            ControllerRule(
                ...
                leaf=ControllerLeaf(
                    leaf_id="l1", action="STOP_USE_CURRENT_EVIDENCE", admitted=True,
                    observed_samples=10, upper_error_bound=0.01,
                ),
            ),
        )
    return ControllerProfile(
        profile_id="profile-x", dataset_sha256="1" * 64, system_manifest_sha256="2" * 64,
        evaluation_protocol_sha256="3" * 64, replay_receipt_sha256="4" * 64,
        training_receipt_sha256="5" * 64, feature_schema_sha256=feature_schema_sha256(),
        corpus_release_id="b" * 16, answer_calibration_id="cal-v1",
        admission_status=admission_status, controller_risk_limit=risk_limit,
        minimum_leaf_samples=minimum_leaf_samples, rules=rules,
    )


class _State:
    def __init__(self, **values: object) -> None:
        self.values = {"top1_score": 0.7, "original_query_has_eligible_evidence": True, "semantic_available": True, **values}
        self.fingerprint = "fp"
        self.original_query_has_eligible_evidence = bool(self.values.get("original_query_has_eligible_evidence"))
...
def test_controller_profile_validation_and_artifact_binding_branches(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="unknown PEC support"):
        ControllerLeaf(
            leaf_id="x", action="ABSTAIN", admitted=False, observed_samples=0,
            upper_error_bound=1.0, support={"bad": FeatureRange()},
        )

    good_leaf = ControllerLeaf(leaf_id="l", action="ABSTAIN", admitted=True, observed_samples=10, upper_error_bound=0.01)
    with pytest.raises(ValueError, match="rule ids"):
        _controller_profile(rules=(ControllerRule(rule_id="r", leaf=good_leaf), ControllerRule(rule_id="r", leaf=ControllerLeaf(leaf_id="l2", action="ABSTAIN", admitted=True, observed_samples=10, upper_error_bound=0.01))))
    with pytest.raises(ValueError, match="leaf ids"):
        _controller_profile(rules=(ControllerRule(rule_id="r1", leaf=good_leaf), ControllerRule(rule_id="r2", leaf=good_leaf)))
    with pytest.raises(ValueError, match="risk limit"):
        _controller_profile(risk_limit=0.01, rules=(ControllerRule(rule_id="r", leaf=ControllerLeaf(leaf_id="l", action="ABSTAIN", admitted=True, observed_samples=10, upper_error_bound=0.02)),))

    profile = _controller_profile()
    path = tmp_path / "profile.json"; path.write_text(profile.canonical_json(), encoding="utf-8")
    ...
    for name in ("dataset", "manifest", "eval", "replay"):
        p = tmp_path / name; p.write_bytes(name.encode()); artifacts.append(p)
        hashes.append(__import__("hashlib").sha256(name.encode()).hexdigest())
    bound = profile.model_copy(update={
        "dataset_sha256": hashes[0], "system_manifest_sha256": hashes[1],
        "evaluation_protocol_sha256": hashes[2], "replay_receipt_sha256": hashes[3],
    })
    bound.validate_artifact_bindings(dataset=artifacts[0], system_manifest=artifacts[1], evaluation_protocol=artifacts[2], replay_receipt=artifacts[3])
    artifacts[0].unlink()
...
def test_predictive_condition_operator_and_support_branches() -> None:
    ...
    assert not _condition_matches(_State(top1_score=True), RuleCondition(feature="top1_score", operator="gt", value=1.0))
    assert not _condition_matches(state, RuleCondition(feature="top1_score", operator="gt", value=True))

    assert _support_failure(_State(top1_score="x"), {"top1_score": FeatureRange(minimum=0.0)}) == "unsupported_non_numeric_feature:top1_score"
    assert _support_failure(_State(top1_score=0.1), {"top1_score": FeatureRange(minimum=0.5)}) == "state_below_support:top1_score"
    assert _support_failure(_State(top1_score=1.1), {"top1_score": FeatureRange(maximum=1.0)}) == "state_above_support:top1_score"
    assert _support_failure(_State(top1_score=0.7), {"top1_score": FeatureRange(minimum=0.5, maximum=1.0)}) is None


def test_predictive_controller_binding_and_out_of_support_branches() -> None:
    state = _State()
    not_admitted = PredictiveEvidenceController(_controller_profile(admission_status="UNKNOWN"), shadow_mode=False).decide(state, corpus_release_id="b" * 16, answer_calibration_id="cal-v1")
    assert not_admitted.fallback_reason == "profile_not_admitted"
    calibration = PredictiveEvidenceController(_controller_profile(), shadow_mode=False).decide(state, corpus_release_id="b" * 16, answer_calibration_id="other")
    assert calibration.fallback_reason == "answer_calibration_mismatch"
    false_rule = ControllerRule(
        rule_id="false", conditions=(RuleCondition(feature="top1_score", operator="lt", value=0.0),),
        leaf=ControllerLeaf(leaf_id="lf", action="ABSTAIN", admitted=True, observed_samples=10, upper_error_bound=0.01),
    )
    out = PredictiveEvidenceController(_controller_profile(rules=(false_rule,)), shadow_mode=False).decide(state, corpus_release_id="b" * 16, answer_calibration_id="cal-v1")
    assert out.fallback_reason == "state_out_of_support"


# --- v0.9.7 coverage closure: PEC research/config/search/knowledge + provenance ---

def test_pec_research_bounds_conditional_and_status_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    bad = research.conditional_risk_report(
        ...
        stratum_key="stratum", error_key="error", risk_limit=1.0, delta=0.1, minimum_samples=1,
    )
    assert bad["status"] == "FAIL" and bad["invalid_rows"] == 2
    good = research.conditional_risk_report(
        [{"stratum": "a", "error": False}], stratum_key="stratum", error_key="error",
        risk_limit=1.0, delta=0.1, minimum_samples=1,
    )
    ...
    assert research.research_status({"status": "PASS"}, [{"status": "UNKNOWN"}]) == ("UNKNOWN", True)


def test_pec_research_ablation_replay_information_and_judgment_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    from korpus.application import pec_research as research
    from korpus.application.pec_training import TrainingRow
    assert research.feature_ablation_generalization([]) == {"status": "UNKNOWN", "reason": "no_training_rows"}
    ...
    assert research.feature_ablation_generalization([row])["reason"] == "nested_full_model_not_estimable"
    calls = 0
    def validation(rows):
        nonlocal calls
        calls += 1
    ...
    assert research.replay_priority_enrichment([{"out_of_support": True}], top_fraction=0)["status"] == "UNKNOWN"
    monkeypatch.setattr(research, "replay_priority", lambda row: 0)
    replay = research.replay_priority_enrichment([{"out_of_support": True}, {"out_of_support": False}], top_fraction=0.5, alpha=1.0)
    ...
    assert research.observed_information_gain([{"query_id": "", "action": "STOP_USE_CURRENT_EVIDENCE"}])["status"] == "UNKNOWN"
    info = research.observed_information_gain([
        {"query_id": "q", "action": "STOP_USE_CURRENT_EVIDENCE", "retrieval_quality": {"x": 1.0}, "search_count": 1},
        {"query_id": "q", "action": "EXPAND", "retrieval_quality": {"x": 2.0}, "gold_hit": True, "quality_ok": True, "search_count": 2},
    ])
    assert info["status"] == "PASS" and info["comparisons"][0]["retrieval_quality_deltas"]["x"] == 1.0

    assert research.production_judgment_validity([{"id": "x", "production_judged": False}])["status"] == "UNKNOWN"
    invalid = research.production_judgment_validity([{"id": "x", "production_judged": True, "judgment_provenance_sha256": "bad", "adjudication_protocol": ""}])
    assert invalid["status"] == "FAIL" and len(invalid["invalid"]) == 2
    valid = research.production_judgment_validity([{"id": "x", "production_judged": True, "judgment_provenance_sha256": "a" * 64, "adjudication_protocol": "p"}])
    assert valid["status"] == "PASS"


def _pec_settings(**updates: object) -> SimpleNamespace:
    base = dict(
        pec_enabled=False, pec_profile_sha256=None, pec_profile_path=None,
        contextual_retrieval_enabled=False, pec_dataset_path=None,
        pec_system_manifest_path=None, pec_evaluation_protocol_path=None,
        pec_replay_receipt_path=None, answer_policy_mode="calibrated",
    )
    base.update(updates)
    return SimpleNamespace(**base)


def test_pec_config_policy_all_admission_branches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match="contextual retrieval"):
        policy.validate_pec_settings(_pec_settings(contextual_retrieval_enabled=True), controlled=True)
    ...
    enabled = _pec_settings(
        pec_enabled=True, pec_profile_path=paths["profile"], pec_dataset_path=paths["dataset"],
        pec_system_manifest_path=paths["manifest"], pec_evaluation_protocol_path=paths["evaluation"],
    ...
    fake = SimpleNamespace(admission_status="UNKNOWN", validate_artifact_bindings=lambda **kwargs: None)
...
def test_repository_search_validation_alias_predicate_and_early_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    from korpus.infrastructure import repository_search as rs
    identity = SimpleNamespace(corpora=frozenset({"c"}))
    with pytest.raises(ValueError, match="positive"):
        rs.search_retrievable_spans(SimpleNamespace(), identity, frozenset({"c"}), date.today(), "q", 0)
    assert rs.search_retrievable_spans(SimpleNamespace(), identity, frozenset({"x"}), date.today(), "q", 1) == []
    assert rs._alias_document_ids(("alpha",), {"doc": ("Alpha beta",), "other": ("gamma",)}) == ["doc"]
    assert len(rs._context_predicates(("alpha",), ["doc"])) == 8
    with pytest.raises(ValueError, match="positive"):
        rs.search_contextual_retrievable_spans(SimpleNamespace(), identity, frozenset({"c"}), date.today(), "q", 0)
    monkeypatch.setattr(rs, "search_retrievable_spans", lambda *a, **k: [(SimpleNamespace(id="s"), None, None)])
    assert len(rs.search_contextual_retrievable_spans(SimpleNamespace(), identity, frozenset({"c"}), date.today(), "q", 1)) == 1
    monkeypatch.setattr(rs, "search_retrievable_spans", lambda *a, **k: [])
    monkeypatch.setattr(rs, "_context_terms", lambda q: ())
    assert rs.search_contextual_retrievable_spans(SimpleNamespace(), identity, frozenset({"c"}), date.today(), "q", 1) == []
    monkeypatch.setattr(rs, "_context_terms", lambda q: ("alpha",))
    monkeypatch.setattr(rs, "_context_predicates", lambda terms, aliases: [])
    assert rs.search_contextual_retrievable_spans(SimpleNamespace(), identity, frozenset({"c"}), date.today(), "q", 1) == []


def test_repository_search_execution_and_baseline_exclusion(monkeypatch: pytest.MonkeyPatch) -> None:
    from korpus.infrastructure import repository_search as rs
    class Result:
        def mappings(self): return self
        def all(self): return [{"span_id": "s2"}]
    class Conn:
    ...
    monkeypatch.setattr(rs.retrieval_queries, "materialize_current", lambda rows, as_of, limit: list(rows)[:limit])
    ...
    monkeypatch.setattr(rs, "search_retrievable_spans", lambda *a, **k: [(SimpleNamespace(id="s1"), None, None)])
    monkeypatch.setattr(rs, "_context_terms", lambda q: ("alpha",))
    monkeypatch.setattr(rs, "_context_predicates", lambda *a: [True])
    result = rs.search_contextual_retrievable_spans(repo, identity, frozenset({"c"}), date.today(), "q", 2)
...
def _mk_binding(*, active: bool = True):
    ...
    return EvidenceBinding(
        document_id="d", version_id="v", span_ids=frozenset({"s"}), source_hash="a" * 64,
...
def test_military_knowledge_validation_publication_and_neighborhood_branches() -> None:
    from korpus.application.military_knowledge import (
        EffectiveGraphIndex, KnowledgeNode, KnowledgeNodeKind, KnowledgeRelation,
        KnowledgeRelationKind, MilitaryKnowledgeGraph, effective_neighborhood,
    )
    ...
    with pytest.raises(ValueError, match="target itself"):
        KnowledgeRelation(source_id="n1", target_id="n1", kind=KnowledgeRelationKind.REQUIRES, bindings=(active,))
    with pytest.raises(ValueError, match="node ids"):
        MilitaryKnowledgeGraph(nodes=(n1, n1))
    rel = KnowledgeRelation(source_id="n1", target_id="n2", kind=KnowledgeRelationKind.SUPERSEDES, bindings=(future,))
    with pytest.raises(ValueError, match="relation identities"):
        MilitaryKnowledgeGraph(nodes=(n1, n2), relations=(rel, rel))
    dangling = KnowledgeRelation(source_id="n1", target_id="missing", kind=KnowledgeRelationKind.REQUIRES, bindings=(active,))
    ...
    assert effective_neighborhood(graph, "n1", as_of=date(2026, 1, 1), relation_kinds=frozenset({KnowledgeRelationKind.REQUIRES})) == ("n1",)
...
def test_military_knowledge_cycle_and_effective_index_traversal() -> None:
    from korpus.application.military_knowledge import (
        EffectiveGraphIndex, KnowledgeNode, KnowledgeNodeKind, KnowledgeRelation,
        KnowledgeRelationKind, MilitaryKnowledgeGraph,
    )
    b = _mk_binding()
    nodes = tuple(KnowledgeNode(id=f"n{i}", kind=KnowledgeNodeKind.DOCTRINE, label=str(i), bindings=(b,)) for i in range(3))
    relations = (
        KnowledgeRelation(source_id="n0", target_id="n1", kind=KnowledgeRelationKind.SUPERSEDES, bindings=(b,)),
        KnowledgeRelation(source_id="n1", target_id="n0", kind=KnowledgeRelationKind.SUPERSEDES, bindings=(b,)),
        KnowledgeRelation(source_id="n1", target_id="n2", kind=KnowledgeRelationKind.REQUIRES, bindings=(b,)),
    )
    graph = MilitaryKnowledgeGraph(nodes=nodes, relations=relations)
    assert any(v.startswith("supersession_cycle") for v in graph.publication_violations(as_of=date(2026, 1, 1)))
    idx = EffectiveGraphIndex.build(graph, as_of=date(2026, 1, 1))
    assert idx.neighborhood("n0", max_depth=3) == ("n0", "n1", "n2")


def test_source_digest_empty_git_tree_falls_back_to_manifest(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_source_digest_inclusion_git_and_archive_validation(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    import subprocess
    ...
    monkeypatch.setattr(module, "_git", lambda *args: (_ for _ in ()).throw(subprocess.CalledProcessError(1, "git")))
    assert module._git_paths("HEAD") is None
    ...
    with pytest.raises(RuntimeError, match="record"):
        module._archive_paths()
    manifest.write_text(json.dumps({"files": [{"path": "missing.txt", "sha256": "0" * 64}]}), encoding="utf-8")
    with pytest.raises(RuntimeError, match="missing or unsafe"):
        module._archive_paths()
    target = tmp_path / "a.txt"; target.write_text("x", encoding="utf-8")
    manifest.write_text(json.dumps({"files": [{"path": "a.txt", "sha256": "0" * 64}]}), encoding="utf-8")
    with pytest.raises(RuntimeError, match="hash mismatch"):
        module._archive_paths()
    digest = __import__("hashlib").sha256(b"x").hexdigest()
    manifest.write_text(json.dumps({"files": [{"path": "a.txt", "sha256": digest}, {"path": "reports/x", "sha256": "x"}]}), encoding="utf-8")
...
def test_pec_training_model_edge_and_candidate_branches() -> None:
    ...
    bool_rows = [tm.TrainingRow("q1", "g", {"original_query_has_eligible_evidence": False}, "A"), tm.TrainingRow("q2", "g", {"original_query_has_eligible_evidence": True}, "B")]
    assert any(c.operator == "eq" for c in tm._candidates(bool_rows))
    str_rows = [tm.TrainingRow("q1", "g", {"top1_score": "uk"}, "A"), tm.TrainingRow("q2", "g", {"top1_score": "en"}, "B")]
    assert any(c.operator == "eq" for c in tm._candidates(str_rows))
    numeric_rows = [tm.TrainingRow(str(i), "g", {"top1_score": float(i)}, "A" if i < 13 else "B") for i in range(30)]
