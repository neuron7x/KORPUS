...
def test_canary_metrics_reject_filter_injection_identifiers() -> None:
    with pytest.raises(ValueError):
        evaluate(
            'proj" OR true', 'api-r2', 'web-r2', minimum_samples=1, maximum_error_rate=0.01,
            window_seconds=60, wait_seconds=0, poll_seconds=1,
        )


@pytest.mark.parametrize(("field", "bad"), [
    ("minimum_samples", True), ("minimum_samples", 1.5), ("minimum_samples", float("nan")),
    ("window_seconds", 60.0), ("window_seconds", float("nan")),
    ("wait_seconds", True), ("wait_seconds", 1.5), ("wait_seconds", float("nan")),
    ("poll_seconds", True), ("poll_seconds", 1.5), ("poll_seconds", float("nan")),
])
def test_canary_policy_rejects_non_discrete_or_nonfinite_controls(field: str, bad: object) -> None:
    ...
    values = dict(project="abcdef", api_revision="a", web_revision="b", minimum_samples=20,
                  maximum_error_rate=0.01, window_seconds=600, wait_seconds=240, poll_seconds=15)
    values[field] = bad
    with pytest.raises(ValueError):
...
def test_canary_request_counter_rejects_noncanonical_values(bad_count: object) -> None:
    payload = {"timeSeries": [{"metric": {"labels": {"response_code_class": "2xx"}},
