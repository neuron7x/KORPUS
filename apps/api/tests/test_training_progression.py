...
def _lesson(lid: str, oid: str, ordinal: int, prereq: str | None = None) -> Lesson:
    binding = SourceBinding(
        id=f"b.{lid}", document_id="doc", version_id="v1", evidence_span_ids=frozenset({"s1"})
    )
    return Lesson(
        id=lid,
        ordinal=ordinal,
...
def _course() -> CourseVersion:
    ...
    return CourseVersion(
        ...
        course_id="c1",
        revision="1",
        modules=(CourseModule(id="m1", ordinal=0, title="Module", lessons=(a, b)),),
    )
...
def test_changed_source_binding_revokes_only_dependent_mastery():
    ...
    changed = invalidate_changed_bindings(
        progress, changed_binding_ids=frozenset({"b.l1"}), now=now
    )
