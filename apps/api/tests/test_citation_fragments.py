...
def test_a_sentence_longer_than_a_citation_is_skipped_not_crashed(
    client: TestClient, admin_identity: Identity
) -> None:
    """`Citation.quote` валідує 1600 символів, і виняток летів повз усі гілки відмови.

    Виміряно 31.08.2026 після переміщення меж прольотів: питання «Як навчити собаку
    команді сидіти?» давало HTTP 500 замість «підстав немає». Падіння на питанні
    користувача гірше за будь-яку відмову, бо не каже нічого.

    Обрізати кандидата не можна: цитата дослівна за побудовою, і обрізана цитата з тим
    самим хешем була б брехнею про джерело. Тому він просто не розглядається.

    ⚠ Цей тест НЕ ізолює сам захист, і це названо: інжест ріже сторінку на шматки не
    довші за 1400 символів, тож речення понад 1600 не виникає з даних, зібраних
    конвеєром. Довгий текст тут теж поділиться на прольоти, і цитата вийде короткою
    незалежно від захисту. Тест тримає ІНВАРІАНТ («жодна цитата не довша за ліміт»), а
    не гілку; мутанта на ту гілку немає навмисно, з причиною в самому коді.
    """
    # Речення мусить ПРОЙТИ поріг покриття, інакше його відсіє інша перевірка й тест
    # буде зеленим від чужої причини. Тому воно складене зі слів самого питання.
    # Одне речення без ЖОДНОГО термінатора всередині: інакше `segment_sentences`
    # поділить його на короткі, і задовгого кандидата не виникне взагалі — тест був би
    # зеленим, не торкнувшись правила.
    long_sentence = (
        "Заборонено відкривати вогонь по цивільній особі "
        + "та за інших обставин, перелічених нижче у цьому ж реченні, " * 30
        + "і на цьому воно нарешті закінчується."
    )
    assert len(long_sentence) > 1600

    answer = _answer(client, admin_identity, long_sentence)

    assert answer is not None, "конвеєр упав замість відмовитись"
    assert all(len(citation.quote) <= 1600 for citation in answer.citations)
