...
def payload():
    return {
        "release": "v0.9.7",
        "revision": "api-00042-x",
        "profile": "prod-v1",
        "phase": "CANARY",
        "environment_class": "PRODUCTION",
        "training_receipt_sha256": D,
    }
...
def test_revision_binding_rejects_release_drift():
    ...
    with pytest.raises(ValueError, match="release binding mismatch"):
        RevisionBinding.from_mapping(row, expected_release="v0.9.7")


def test_revision_binding_rejects_nonproduction_environment():
    row = payload()
    row["environment_class"] = "PRODUCTION_LIKE"
    with pytest.raises(ValueError, match="environment_class=PRODUCTION"):
        RevisionBinding.from_mapping(row, expected_release="v0.9.7")


def test_revision_binding_rejects_unbound_training_receipt():
    row = payload()
    row["training_receipt_sha256"] = "bad"
