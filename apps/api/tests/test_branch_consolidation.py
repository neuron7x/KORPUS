...
def test_the_head_is_the_one_the_deployed_base_expects() -> None:
    ...
    Оновлено 07.09.2026 при інтеграції PR#44: `0024_capability_effect_ledger` створює
    таблицю `capability_effects` під керований шар спроможностей. Розгорнута база мусить
    мігрувати ПЕРЕД підняттям нового коду — як і завжди, коли ланцюг довшає, — але сама
    міграція розширювальна: наявних таблиць вона не торкається, тож ревізія N-1 лишається
    сумісною зі схемою.
    """
    assert alembic_heads() == ["0024_capability_effect_ledger.py"]
