...
#
# Корпус тримає два українські документи про тактичну медицину, і обидва — документи ПРО
# предмет, не документи предмета: наказ із НАЗВАМИ втручань («Використання турнікета для
# зупинки кровотечі») і силабус навчальної дисципліни («слухачі повинні знати»). Термін
# наявний, читач діяти не може.
#
# Детектор мусить розділяти РІД тексту, і довести це можна лише парою: він зобов'язаний
# ловити норму І пропускати опис норми. Одна половина без другої — це або сито, що
# ловить усе, або сито, що не ловить нічого.

NORMATIVE = "Чатовий зобов'язаний охороняти пост і нікому не дозволяється його залишати."
CURRICULAR = (
    "Предметом вивчення навчальної дисципліни є алгоритми надання допомоги; "
    "удосконалення слухачами практичних навичок накладання турнікета."
)


def test_a_normative_span_is_actionable() -> None:
    assert actionable("пост", [NORMATIVE.lower()]) == (1, 1)


def test_a_syllabus_that_names_the_intervention_is_not() -> None:
    """Силабус називає турнікет і не каже про нього нічого, що можна виконати."""
    assert actionable("турнікет", [CURRICULAR.lower()]) == (1, 0)


def test_a_term_the_corpus_does_not_hold_holds_no_spans() -> None:
    """Негативний контроль: придатність не сміє вигадуватись там, де немає й терміна."""
    assert actionable("тампонад", [NORMATIVE.lower(), CURRICULAR.lower()]) == (0, 0)


def test_the_directive_set_is_narrow_enough_to_separate() -> None:
    """Ширший набір зворотів підняв би «придатність», не змінивши корпусу.

    Тут пінеться саме РОЗДІЛЬНІСТЬ: набір, у який дописали звичайне слово, почав би
    ловити й опис. Виміряно на всьому корпусі 01.09.2026: базовий рівень 0.0702, а
    «чатов» 0.53, «наказ» 0.42 — вшестеро над ним. Медична частина 0 із 26.
    """
    ordinary = [
        "слухачі вивчають курс тактичної медицини протягом семестру.",
        "перелік втручань наведено у додатку до наказу.",
        "опис курсу та веб-сайт його розміщення.",
    ]

    assert all(actionable("к", [span])[1] == 0 for span in ordinary)
