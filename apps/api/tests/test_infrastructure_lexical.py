...
async def test_query_text_actually_selects_documents() -> None:
    ...
    found = await retriever.search(Query(text="порядок евакуації"), ALL_TIERS, GRANTED)
...
async def test_a_query_matching_nothing_returns_nothing() -> None:
    ...
    assert await retriever.search(Query(text="радіочастотний план"), ALL_TIERS, GRANTED) == []


async def test_stopword_only_query_matches_nothing() -> None:
    retriever = LexicalRetriever([make_span(text="Правила зберігання пального")])
    assert await retriever.search(Query(text="і не та"), ALL_TIERS, GRANTED) == []
...
async def test_score_reflects_term_coverage() -> None:
    ...
    partial = await retriever.search(Query(text="порядок ремонту"), ALL_TIERS, GRANTED)
...
async def test_tiers_are_filtered_inside_the_index() -> None:
    ...
    found = await retriever.search(Query(text="порядок"), PUBLIC_ONLY, GRANTED)
    assert [span.chunk_id for span in found] == [public.chunk_id]


async def test_a_corpus_outside_the_grant_is_never_searched() -> None:
    """The bypass this test exists for: the caller names nothing and gets everything."""
    other_corpus = uuid4()
    inside = make_span(text="порядок дій")
    outside = make_span(text="порядок дій", corpus_id=other_corpus)
    retriever = LexicalRetriever([inside, outside])
    found = await retriever.search(Query(text="порядок"), ALL_TIERS, GRANTED)
    assert [span.chunk_id for span in found] == [inside.chunk_id]


async def test_an_empty_grant_searches_nothing() -> None:
    retriever = LexicalRetriever([make_span(text="порядок дій")])
    assert await retriever.search(Query(text="порядок"), ALL_TIERS, frozenset()) == []
...
async def test_limit_is_honoured() -> None:
    ...
    assert len(await retriever.search(Query(text="порядок"), ALL_TIERS, GRANTED, limit=2)) == 2


async def test_search_is_deterministic_across_repeated_calls() -> None:
    spans = [make_span(text="порядок дій") for _ in range(4)]
    retriever = LexicalRetriever(spans)
    first = await retriever.search(Query(text="порядок"), ALL_TIERS, GRANTED)
    second = await retriever.search(Query(text="порядок"), ALL_TIERS, GRANTED)
    assert [s.chunk_id for s in first] == [s.chunk_id for s in second]


async def test_an_added_span_carries_its_own_corpus_membership() -> None:
    other = uuid4()
    granted = make_span(text="порядок дій")
    retriever = LexicalRetriever()
    retriever.add(granted)
    retriever.add(make_span(text="порядок дій", corpus_id=other))
    found = await retriever.search(Query(text="порядок"), ALL_TIERS, GRANTED)
    assert [span.chunk_id for span in found] == [granted.chunk_id]
...
async def test_added_spans_are_searchable_and_counted() -> None:
    ...
    assert len(await retriever.search(Query(text="порядок"), ALL_TIERS, GRANTED)) == 1
