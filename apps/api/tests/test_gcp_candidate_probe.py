...
def test_candidate_target_requires_one_immutable_tagged_revision() -> None:
    payload = {"status": {"traffic": [
        ...
        {"revisionName": "api-00002", "percent": 0, "tag": "candidate", "url": "https://candidate---api-xyz.run.app"},
    ]}}
    assert candidate_target.candidate(payload) == {
        "tag": "candidate", "revision": "api-00002", "url": "https://candidate---api-xyz.run.app", "percent": 0,
    }
    with pytest.raises(ValueError):
        candidate_target.candidate({"status": {"traffic": []}})
    for revision, url in (("API_BAD", "https://candidate---api-xyz.run.app"), ("api-r2", "https://evil.example")):
        with pytest.raises(ValueError):
            candidate_target.candidate({"status": {"traffic": [{"revisionName": revision, "percent": 0, "tag": "candidate", "url": url}]}})


def test_probe_rejects_non_run_app_targets() -> None:
    for bad in ("http://x.run.app", "https://user:pass@x.run.app", "https://evil.example", "https://x.run.app?token=x"):
        with pytest.raises(ValueError):
...
def test_probe_requires_exact_ready_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    checks = probe_candidate.evaluate(
        "https://candidate---api-xyz.run.app", "https://candidate---web-xyz.run.app", attempts=2, timeout=1
    )
