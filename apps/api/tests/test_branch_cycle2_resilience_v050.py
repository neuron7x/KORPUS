...
def test_anchor_codec_file_monotonic_cleanup_and_reset(
    ...
    monkeypatch.setattr(
        audit_anchor.os, "replace", lambda *a, **k: (_ for _ in ()).throw(OSError("replace"))
    )
...
def test_http_anchor_error_read_404_and_close_matrix() -> None:
    ...
    audit_anchor.HttpAuditAnchorStore(
        "https://a.example",
        b"k" * 32,
        client=SimpleNamespace(get=lambda *a: Resp(404), put=lambda *a, **k: Resp(204)),
    ).close()
...
def test_oidc_verify_branch_matrix(monkeypatch: pytest.MonkeyPatch) -> None:
    client = SimpleNamespace(
        get_signing_key_from_jwt=lambda token: SimpleNamespace(key="key"), close=lambda: None
    )
...
def _service(repo=None, policy=None, **kw) -> IngestionService:
    return IngestionService(
        repo or SimpleNamespace(),
        SimpleNamespace(),
        policy or SimpleNamespace(),
        ExtractionSettings(ocr_enabled=False, ocr_languages="eng"),
        SimpleNamespace(),
        **kw,
    )
...
def _actor(subject="reviewer") -> Identity:
    return Identity(
        subject=subject,
        roles=frozenset({"reviewer", "curator", "admin"}),
...
class Policy:
    ...
    def require(self, actor, permission):
        del actor, permission

    def can_access_document(self, actor, document):
        del actor, document
        return SimpleNamespace(allowed=self.allowed)


class Repo:
    def __init__(self, version=None, document=None):
        self.version, self.document = version, document

    def get_version(self, actor, id):
        del actor, id
        return self.version

    def get_document(self, actor, id):
        del actor, id
        return self.document

    def transition_version(self, *a, **k):
        return "ok"
...
def _version(state=ReviewState.QUARANTINED, **kw):
    base = dict(
        ...
        in_force_from=datetime(2026, 1, 1, tzinfo=UTC).date(),
...
def test_ingestion_transition_refusal_matrix() -> None:
    ...
    with pytest.raises(LookupError, match="version"):
        _service(Repo(None, None), Policy()).transition(
            actor, uuid4(), _transition(ReviewState.METADATA_REVIEWED)
        )
    v = _version()
    with pytest.raises(LookupError, match="document"):
        _service(Repo(v, None), Policy()).transition(
            actor, v.id, _transition(ReviewState.METADATA_REVIEWED)
        )
    with pytest.raises(PermissionError, match="access"):
        _service(Repo(v, SimpleNamespace()), Policy(False)).transition(
            actor, v.id, _transition(ReviewState.METADATA_REVIEWED)
        )
    with pytest.raises(ValueError, match="invalid review transition"):
        _service(Repo(_version(ReviewState.REJECTED), SimpleNamespace()), Policy()).transition(
            actor, v.id, _transition(ReviewState.METADATA_REVIEWED)
        )
    nonnorm = _version(ReviewState.CONTENT_REVIEWED, authority=AuthorityClass.ADVERSARY)
    with pytest.raises(ValueError, match="cannot be approved"):
        _service(Repo(nonnorm, SimpleNamespace()), Policy()).transition(
            actor, nonnorm.id, _transition(ReviewState.APPROVED)
        )
    nodate = _version(ReviewState.CONTENT_REVIEWED, in_force_from=None)
    with pytest.raises(ValueError, match="effective_from"):
        _service(Repo(nodate, SimpleNamespace()), Policy()).transition(
            actor, nodate.id, _transition(ReviewState.APPROVED)
        )
    ...
    with pytest.raises(ValueError, match="content reviewer"):
        _service(
            Repo(same, SimpleNamespace()), Policy(), review_separation_required=True
        ).transition(actor, same.id, _transition(ReviewState.CONTENT_REVIEWED))
    prior = _version(ReviewState.CONTENT_REVIEWED, metadata_reviewed_by=actor.subject)
    with pytest.raises(ValueError, match="approver"):
        _service(
            Repo(prior, SimpleNamespace()), Policy(), review_separation_required=True
        ).transition(actor, prior.id, _transition(ReviewState.APPROVED))
    valid = _version(ReviewState.QUARANTINED)
    service = _service(
        Repo(valid, SimpleNamespace()),
        Policy(),
        require_reviewer_credentials=True,
        reviewer_registry=SimpleNamespace(authorize=lambda **k: "cred"),
    )
