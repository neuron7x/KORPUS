...
def test_service_level_shape_fail_closed_matrix() -> None:
    ...
    result = service_levels.evaluate_load_slos({"soak": {"p95_seconds": 1, "refusal_reasons": [], "decisions": []}, "cold_first_request": {"seconds": 1}})
    assert result["load_slo_no_subject_throttle_rated"] and result["load_slo_no_retrieval_deadline"]
    result = service_levels.evaluate_load_slos({"soak": {"p95_seconds": 1, "statuses": {"500": 1}, "refusal_reasons": {service_levels.SUBJECT_THROTTLE_REASON: 1}, "decisions": {"retrieval_deadline_exceeded": 1}}, "cold_first_request": {"seconds": 1}})
    assert not result["load_slo_no_5xx_rated"] and not result["load_slo_no_subject_throttle_rated"] and not result["load_slo_no_retrieval_deadline"]
    ...
    assert service_levels.evaluate_load_slos({
        "soak": {"p95_seconds": 1, "statuses": {"500": -1}},
        "cold_first_request": {"seconds": 1},
    })["load_slo_no_5xx_rated"] is False


class Accounts:
    def __init__(self, account): self.account = account
    def get_account(self, account_id): del account_id; return self.account


class Subs:
    def __init__(self, plan): self.plan = plan
    def get_plan_by_code(self, code): del code; return self.plan


class Service:
    def __init__(self, active=None): self.active = active
    def active_subscription(self, account_id): del account_id; return self.active
    def start_subscription(self, actor, account, plan): del actor, account, plan; return SimpleNamespace(id=uuid4())


class Provider:
    name = "fake"
    def create_checkout(self, **kwargs):
        return checkout.CheckoutDescriptor(kwargs["subscription"].id, "fake", "https://pay", "POST", {})
...
def test_checkout_fail_closed_branch_matrix() -> None:
    ...
    with pytest.raises(checkout.CheckoutUnavailable, match="account does not exist"):
        checkout.CheckoutService(Accounts(None), Subs(None), Service(), Provider(), "https://k").start("u", aid, "p")
    account = SimpleNamespace(id=aid)
    with pytest.raises(checkout.CheckoutUnavailable, match="active subscription"):
        checkout.CheckoutService(Accounts(account), Subs(None), Service(active=object()), Provider(), "https://k").start("u", aid, "p")
    with pytest.raises(checkout.PlanNotFound):
        checkout.CheckoutService(Accounts(account), Subs(None), Service(), Provider(), "https://k").start("u", aid, "p")
    ...
    with pytest.raises(checkout.CheckoutUnavailable, match="sellable price"):
        checkout.CheckoutService(Accounts(account), Subs(unsellable), Service(), Provider(), "https://k").start("u", aid, "p")
    ...
    with pytest.raises(checkout.CheckoutUnavailable, match="public base URL"):
        checkout.CheckoutService(Accounts(account), Subs(sellable), Service(), Provider(), "").start("u", aid, "p")
    descriptor = checkout.CheckoutService(Accounts(account), Subs(sellable), Service(), Provider(), "https://k/").start("u", aid, "p")
...
def test_scanner_config_missing_size_and_response_matrix(tmp_path: Path) -> None:
    ...
    class Conn:
        ...
        def recv(self, n): del n; return next(self.chunks, b"")
...
class Body:
    def __init__(self, data: bytes, close=None): self.io = BytesIO(data); self.close = close
    def read(self, n=-1): return self.io.read(n)
...
class S3:
    ...
    def head_object(self, **kw):
        ...
        if key not in self.objects:
            e=RuntimeError("missing"); e.response={"Error":{"Code":"404"}}; raise e
        return self.objects[key]["head"]
    def put_object(self, **kw):
        body=kw["Body"]; data=body.read() if hasattr(body,"read") else bytes(body)
        self.objects[kw["Key"]]={"data":data,"head":{"Metadata":kw["Metadata"],"ContentLength":len(data),"ChecksumSHA256":kw["ChecksumSHA256"]}}
    def get_object(self, **kw):
        item=self.objects[kw["Key"]]
        return {"Body": Body(item["data"], None), "Metadata": item["head"]["Metadata"], "ContentLength":len(item["data"]), "ChecksumSHA256":item["head"]["ChecksumSHA256"]}
...
def test_remaining_object_store_close_and_put_path_edges(tmp_path: Path) -> None:
    data=b"abc"; digest=_digest(data); p=tmp_path/"p"; p.write_bytes(data)
    client=S3(); store=osmod.S3ObjectStore(bucket="bucket", prefix="objects", client=client, max_object_bytes=64)
    key=store.put_path(p,digest,"x")
    assert store.get(key)==data  # non-callable close normal path
    out=tmp_path/"out"; store.get_to_path(key,out); assert out.read_bytes()==data

    # callable close on declared-oversize path
    closed=[]
    store.client.get_object=lambda **kw: {"Body": Body(data, lambda: closed.append(True)), "Metadata":{"sha256":digest}, "ContentLength":999}
    with pytest.raises(RuntimeError, match="read limit"):
        store.get_to_path(key,tmp_path/"big")
    assert closed


def test_pdf_ocr_renderer_and_image_loop_edges(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    source=tmp_path/"x.pdf"; source.write_bytes(b"pdf")
    calls=[]
    def run(args, **kwargs):
        del kwargs
        calls.append(args[0])
