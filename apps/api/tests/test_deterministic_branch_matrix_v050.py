...
from korpus.application import deployment
from korpus.application import release_ledger as ledger
from korpus.application import retrieval
...
from korpus.infrastructure.liqpay import LiqPayBillingProvider, _amount_minor, _mapped_status, _provider_datetime
...
def test_config_policy_auth_and_controlled_matrix(monkeypatch: pytest.MonkeyPatch) -> None:
    with pytest.raises(ValueError, match="OIDC authentication"):
        config_policy._validate_auth(_cfg(environment="production", auth_mode="dev"), controlled=True)
    with pytest.raises(ValueError, match="acknowledgement"):
        config_policy._validate_auth(_cfg(auth_mode="dev", dev_mode_acknowledgement="no"), controlled=False)
    ...
    monkeypatch.setattr(config_policy, "first_unmet", lambda settings: SimpleNamespace(message="missing-X"))
    with pytest.raises(ValueError, match="missing-X"):
        config_policy._validate_controlled_requirements(_cfg(), controlled=True)


def test_config_policy_browser_model_semantic_and_runtime_matrix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match="requires OIDC"):
        config_policy._validate_browser_oidc(_cfg(browser_auth_enabled=True, auth_mode="jwt"), controlled=False)
    with pytest.raises(ValueError, match="endpoints"):
        config_policy._validate_browser_oidc(_cfg(browser_auth_enabled=True, oidc_client_id=""), controlled=False)
    with pytest.raises(ValueError, match="32 characters"):
        config_policy._validate_browser_oidc(_cfg(browser_auth_enabled=True, resolved_browser_session_key="x"), controlled=False)
    with pytest.raises(ValueError, match="must use HTTPS"):
        config_policy._validate_browser_oidc(_cfg(browser_auth_enabled=True, oidc_token_endpoint="http://id/token"), controlled=False)
    with pytest.raises(ValueError, match="redirect URI"):
        config_policy._validate_browser_oidc(_cfg(browser_auth_enabled=True, oidc_redirect_uri="http://remote/cb"), controlled=False)
    ...
    with pytest.raises(ValueError, match="refused in a controlled"):
        config_policy._validate_model_integrations(_cfg(answer_composer_enabled=True, environment="controlled"))
    monkeypatch.setattr(config_policy, "resolved_model_api_key", lambda settings: None)
    ...
    with pytest.raises(ValueError, match="refused in a isolated"):
        config_policy._validate_model_integrations(_cfg(query_planner_enabled=True, environment="isolated"))
    config_policy._validate_model_integrations(_cfg())

    with pytest.raises(ValueError, match="PostgreSQL"):
        config_policy._validate_semantic_retrieval(_cfg(semantic_retrieval_enabled=True), controlled=False)
    with pytest.raises(ValueError, match="endpoint and model"):
        config_policy._validate_semantic_retrieval(_cfg(semantic_retrieval_enabled=True, database_url="postgresql://db"), controlled=False)
    with pytest.raises(ValueError, match="positive semantic weight"):
        config_policy._validate_semantic_retrieval(_cfg(semantic_retrieval_enabled=True, database_url="postgresql://db", embedding_endpoint="https://e", embedding_model_id="m", semantic_weight=0), controlled=False)
    with pytest.raises(ValueError, match="must use HTTPS"):
        config_policy._validate_semantic_retrieval(_cfg(semantic_retrieval_enabled=True, database_url="postgresql://db", embedding_endpoint="http://e", embedding_model_id="m", semantic_weight=.1), controlled=True)
    with pytest.raises(ValueError, match="requires authentication"):
        config_policy._validate_semantic_retrieval(_cfg(semantic_retrieval_enabled=True, database_url="postgresql://db", embedding_endpoint="https://e", embedding_model_id="m", semantic_weight=.1), controlled=True)
    config_policy._validate_semantic_retrieval(_cfg(semantic_retrieval_enabled=True, database_url="postgresql://db", embedding_endpoint="https://e", embedding_model_id="m", semantic_weight=.1, resolved_embedding_token="t"), controlled=True)
    with pytest.raises(ValueError, match="must be zero"):
        config_policy._validate_semantic_retrieval(_cfg(semantic_weight=.1), controlled=False)
    config_policy._validate_semantic_retrieval(_cfg(), controlled=False)

    with pytest.raises(ValueError, match="audit_anchor_url"):
        config_policy._validate_runtime_integrations(_cfg(audit_anchor_mode="http"), controlled=False)
    ...
    with pytest.raises(ValueError, match="JWT secret"):
        config_policy._validate_runtime_integrations(_cfg(auth_mode="jwt", resolved_jwt_secret="replace-me"), controlled=False)
    with pytest.raises(ValueError, match="overlap"):
        config_policy._validate_runtime_integrations(_cfg(chunk_overlap_chars=100), controlled=False)
...
def _ev(status: str = "PASS", cls: ac.EvidenceClass = ac.EvidenceClass.EXECUTED, **kw) -> ac.EvidencePoint:
    ...
    return ac.EvidencePoint(cls, kw.pop("source_digest", SOURCE), kw.pop("release", RELEASE), status, executed, negative, independent, attested)
...
def test_assurance_validation_and_ceiling_matrix() -> None:
    ...
    with pytest.raises(ValueError, match="negative-control"):
        ac.EvidencePoint(ac.EvidenceClass.EXECUTED_WITH_NEGATIVE_CONTROL, SOURCE, RELEASE, "PASS", True, False)
    with pytest.raises(ValueError, match="independent attested"):
        ac.EvidencePoint(ac.EvidenceClass.INDEPENDENT_ATTESTED, SOURCE, RELEASE, "PASS", True, True, False, True)
    ...
    with pytest.raises(ValueError, match="dimension ids"):
        ac.ReadinessPolicy((ac.DimensionPolicy("x", .5), ac.DimensionPolicy("x", .5)), ())
    with pytest.raises(ValueError, match="gate ids"):
        ac.ReadinessPolicy((ac.DimensionPolicy("x", 1.0),), (ac.GateRequirement("g"), ac.GateRequirement("g")))

    p = ac.DimensionPolicy("x", 1.0)
    assert ac.evidence_ceiling(p, ac.EvidencePoint(ac.EvidenceClass.DECLARATIVE, SOURCE, RELEASE, "PASS")) == 70
    ...
    assert ac.calibrated_dimension_score(p, ac.DimensionObservation(99, _ev("FAIL")), source_digest=SOURCE, release=RELEASE) == 0


def test_assurance_gate_and_join_matrix() -> None:
    p = ac.ReadinessPolicy((ac.DimensionPolicy("x", 1.0),), (ac.GateRequirement("g", require_negative_control=True, require_independent=True, require_attestation=True),))
    ok, failures = ac.evaluate_gate(p.mandatory_gates[0], None, source_digest=SOURCE, release=RELEASE)
    assert not ok and failures == ("g.missing",)
    weak = _ev(cls=ac.EvidenceClass.EXECUTED, source_digest="b" * 64, release="v9")
    ok, failures = ac.evaluate_gate(p.mandatory_gates[0], weak, source_digest=SOURCE, release=RELEASE)
...
def _ledger_event(**changes) -> ledger.ReleaseLedgerEvent:
    base = dict(sequence=1, release_identity_digest=SOURCE, release=RELEASE, from_stage="DRAFT", to_stage="INTEGRATED", author_subject="author", verifier_subject=None, gate_set_sha256="b" * 64, timestamp="2026-08-15T12:00:00Z", previous_event_sha256="0" * 64, withdrawal_reason=None, event_sha256="")
    base.update(changes)
...
def test_release_ledger_validation_and_integrity_matrix() -> None:
    ...
    failures, _ = ledger._event_integrity_failures(1, bad_sequence, previous_hash="f" * 64, previous_to="VERIFIED", previous_time=datetime(2026, 8, 15, 13, tzinfo=UTC))
    assert {"event[1].sequence", "event[1].previous_hash", "event[1].hash", "event[1].stage_continuity", "event[1].timestamp_monotonicity"}.issubset(failures)
    jump = _ledger_event(from_stage="DRAFT", to_stage="VERIFIED").with_hash()
    failures, _ = ledger._event_integrity_failures(1, jump, previous_hash="0" * 64, previous_to=None, previous_time=None)
    ...
    failures, _ = ledger._event_integrity_failures(1, after_withdraw, previous_hash="0" * 64, previous_to=None, previous_time=None)
    assert "event[1].transition_after_withdrawal" in failures
    assert ledger._identity_failures(1, first, identity="c" * 64, release="v9") == ["event[1].release_identity", "event[1].release"]
...
def test_browser_session_fail_closed_matrix() -> None:
    ...
    with pytest.raises(BrowserSessionError, match="type mismatch"):
        codec.open(_sealed(codec, {"v": 2, "kind": "session", "iat": 900, "exp": 1100, "payload": {}}), expected_kind="session")
    with pytest.raises(BrowserSessionError, match="timestamps"):
        codec.open(_sealed(codec, {"v": 1, "kind": "session", "iat": "900", "exp": 1100, "payload": {}}), expected_kind="session")
    for iat, exp in ((1031, 1100), (900, 1000)):
        with pytest.raises(BrowserSessionError, match="expired"):
            codec.open(_sealed(codec, {"v": 1, "kind": "session", "iat": iat, "exp": exp, "payload": {}}), expected_kind="session")
    with pytest.raises(BrowserSessionError, match="payload"):
        codec.open(_sealed(codec, {"v": 1, "kind": "session", "iat": 900, "exp": 1100, "payload": []}), expected_kind="session")
    assert codec.open(_sealed(codec, {"v": 1, "kind": "session", "iat": 900, "exp": 1100, "payload": {"x": 1}}), expected_kind="session") == {"x": 1}


class _TokenResponse:
    def __init__(self, payload, *, fail=False): self.payload=payload; self.fail=fail
    def raise_for_status(self):
        if self.fail: raise httpx.HTTPError("fail")
    def json(self): return self.payload


class _OIDCTransport:
    def __init__(self, response): self.response=response; self.posts=[]; self.closed=False
    def post(self, url, data=None, headers=None): self.posts.append((url,data,headers)); return self.response
    def close(self): self.closed=True


def _oidc_client(response, **kw):
    return BrowserOIDCClient(authorization_endpoint="https://id/a", token_endpoint="https://id/t", client_id="c", redirect_uri="https://app/cb", scopes=["profile", "openid"], client=_OIDCTransport(response), **kw)


def test_browser_oidc_constructor_and_exchange_matrix() -> None:
    with pytest.raises(ValueError, match="HTTPS"):
        BrowserOIDCClient(authorization_endpoint="http://id/a", token_endpoint="https://id/t", client_id="c", redirect_uri="x", scopes=[])
    with pytest.raises(ValueError, match="client id"):
        BrowserOIDCClient(authorization_endpoint="https://id/a", token_endpoint="https://id/t", client_id="", redirect_uri="x", scopes=[])
    client = _oidc_client(_TokenResponse({"access_token":"a","id_token":"i","expires_in":300}), client_secret="secret")
    ...
    for payload, message in [
        ...
        with pytest.raises(BrowserSessionError, match=message): bad.exchange("c","v")
    failed = _oidc_client(_TokenResponse({}, fail=True))
    with pytest.raises(BrowserSessionError, match="exchange failed"): failed.exchange("c","v")
...
def test_deployment_patch_branch_matrix(tmp_path: Path) -> None:
    ...
    deployment._apply_json6902(doc, [
...
def test_liqpay_fail_closed_and_conversion_matrix() -> None:
    for args in [("", "k"), ("p", "")]:
        with pytest.raises(ValueError, match="keys"): LiqPayBillingProvider(*args)
    with pytest.raises(ValueError, match="algorithm"): LiqPayBillingProvider("p","k",signature_algorithm="md5")
    sha1 = LiqPayBillingProvider("p","k",signature_algorithm="sha1")
    assert sha1.sign_data("x")
    p = LiqPayBillingProvider("p","k")
    with pytest.raises(ValueError, match="unsigned"): p.verify_event(b"x", None)
    with pytest.raises(ValueError, match="ASCII"): p.verify_event(b"\xff", "x")
    with pytest.raises(ValueError, match="signature"): p.verify_event(b"e30=", "x")
    bad = base64.b64encode(b"not-json")
    with pytest.raises(ValueError, match="malformed"): p.verify_event(bad, p.sign_data(bad.decode()))
    arr = base64.b64encode(b"[]")
    with pytest.raises(ValueError, match="not an object"): p.verify_event(arr, p.sign_data(arr.decode()))
    wrong = base64.b64encode(json.dumps({"public_key":"other"}).encode())
    with pytest.raises(ValueError, match="another merchant"): p.verify_event(wrong, p.sign_data(wrong.decode()))
    good = base64.b64encode(json.dumps({"public_key":"p","action":"regular","status":"success","payment_id":123}).encode())
    event=p.verify_event(good,p.sign_data(good.decode()))
    assert p.event_identity(event)[0].startswith("123:")
    with pytest.raises(ValueError, match="stable transaction"): p.event_identity({"action":"x","status":"y"})
    with pytest.raises(Exception): p.subscription_view({"action":"regular","status":"wait"})
    with pytest.raises(ValueError, match="order_id"): p.subscription_view({"action":"regular","status":"success"})
    assert _mapped_status("regular","unsubscribed") == "canceled"
    with pytest.raises(ValueError, match="unsupported"): _mapped_status("x","success")
    assert _amount_minor(None) is None and _amount_minor("") is None and _amount_minor("1.23") == 123
    for value, message in [("bad","numeric"),("1.001","sub-minor"),(0,"positive")]:
        with pytest.raises(ValueError, match=message): _amount_minor(value)
...
def test_models_validation_and_retrieval_parameter_matrix() -> None:
    ...
    with pytest.raises(ValueError): Identity(subject="x\ny", corpora=frozenset({"public"}))
    with pytest.raises(ValueError, match="at least one corpus"): Identity(subject="x", corpora=frozenset())
    with pytest.raises(ValueError, match="compartment"): DocumentCreate(canonical_title="abc", issuer="ii", compartments=frozenset({"BAD SPACE"}))
    with pytest.raises(ValueError, match="effective_until"): VersionCreate(revision="1", effective_from=date(2026,2,1), effective_until=date(2026,1,1))
    span_id=uuid4(); version_id=uuid4()
    with pytest.raises(ValueError, match="text_hash"): EvidenceSpanRecord(version_id=version_id, ordinal=0, text="x", text_hash="a"*64)
    span=EvidenceSpanRecord(version_id=version_id, ordinal=0, text="x")
    assert span.text_hash == hashlib.sha256(b"x").hexdigest()
    with pytest.raises(ValueError, match="NUL"): QueryRequest(text="abc\x00def")
    quote="quoted"; qh=hashlib.sha256(quote.encode()).hexdigest()
    base=dict(document_id=uuid4(),version_id=version_id,span_id=span_id,title="t",revision="1",quote=quote,quote_start=0,quote_end=len(quote),quote_hash=qh,source_hash="b"*64)
    with pytest.raises(ValueError, match="quote_end"): Citation(**{**base,"quote_start":2,"quote_end":2})
    with pytest.raises(ValueError, match="quote_hash"): Citation(**{**base,"quote_hash":"c"*64})
    assert Citation(**base).quote == quote

    for k1,b in ((0.09,.5),(4.1,.5),(1.5,-.1),(1.5,1.1)):
        with pytest.raises(ValueError): retrieval.BM25Parameters(k1,b)
    with pytest.raises(ValueError, match=r"\[0, 1\]"): retrieval.RetrievalWeights(lexical=1.1,semantic=0,query_coverage=0,character=0,authority=0,phrase=0,temporal=0)
    with pytest.raises(ValueError, match="sum to 1"): retrieval.RetrievalWeights(lexical=.1,semantic=0,query_coverage=0,character=0,authority=0,phrase=0,temporal=0)
    ...
    assert retrieval.jaccard(frozenset({"x"}), frozenset({"x","y"})) == .5
...
def test_retrieval_scoring_validation_and_edge_matrix() -> None:
    with pytest.raises(ValueError, match="equal length"): retrieval.score_candidates("q",["a"],[True,False])
    with pytest.raises(ValueError, match="component arrays"): retrieval.score_candidates("query",["text"],authority_scores=[])
    with pytest.raises(ValueError, match=r"\[0, 1\]"): retrieval.score_candidates("query",["text"],authority_scores=[2])
    assert retrieval.score_candidates("the and", ["text"]) == []
    assert retrieval.score_candidates("query", []) == []
    scored = retrieval.score_candidates("alpha beta", ["", "alpha beta", "gamma"], semantic_scores=[0,.5,0], temporal_scores=[0,.5,0])
