...
def test_registry_loader_absent_and_invalid(
    ...
    monkeypatch.setattr(
        admission.AttestorRegistry, "load", lambda path: (_ for _ in ()).throw(ValueError("bad"))
    )
    assert admission._load_registry(tmp_path) is None
...
@pytest.mark.parametrize(
    ...
    [
        123,
        "",
        "x" * (query_plan.MAX_QUERY_CHARS + 1),
        "hello\nworld",
        "hello?",
        " ".join(["w"] * (query_plan.MAX_QUERY_TOKENS + 1)),
...
def test_retrieval_validation_matrix() -> None:
    ...
    with pytest.raises(ValueError, match="cover every"):
        retrieval.HybridLexicalRetriever(
            SimpleNamespace(), authority_priors={next(iter(retrieval.AUTHORITY_PRIOR)): 0.5}
        )
...
def _request(*, host="testclient", cookies=None, codec=None, verifier=None):
    ...
    return SimpleNamespace(
        client=SimpleNamespace(host=host), cookies=cookies or {}, app=app, state=req_state
    )
...
def test_auth_dev_oidc_and_bearer_refusal_matrix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        auth, "load_entitlement_profile", lambda *a, **k: (_ for _ in ()).throw(ValueError("bad"))
    )
    with pytest.raises(HTTPException) as exc:
        auth._oidc_identity(
            {}, _auth_settings(auth_mode="oidc", entitlement_profile_path=Path("/x"))
        )
    assert exc.value.status_code == 403
    ...
    bad_codec = SimpleNamespace(
        open=lambda *a, **k: (_ for _ in ()).throw(BrowserSessionError("bad"))
    )
    with pytest.raises(HTTPException) as exc:
        auth.get_identity(
            None,
            _auth_settings(auth_mode="oidc", browser_auth_enabled=True),
            _request(cookies={"korpus_session": "x"}, codec=bad_codec),
        )
    assert exc.value.status_code == 401
    ...
    with pytest.raises(HTTPException) as exc:
        auth.get_identity(
            None,
            _auth_settings(auth_mode="oidc", browser_auth_enabled=True),
            _request(cookies={"korpus_session": "x"}, codec=empty_codec),
        )
    assert exc.value.status_code == 401
    ...
    monkeypatch.setattr(
        auth,
        "_identity_from_local_claims",
        lambda claims: Identity(
            subject="jwt",
...
def test_ingestion_job_target_validator_matrix() -> None:
    ...
    with pytest.raises(ValueError, match="document_id only"):
        IngestionJobRecord(
            kind=IngestionJobKind.VERSION, document=document, document_id=uuid4(), **common
        )
    with pytest.raises(ValueError, match="document_id only"):
...
def test_release_ledger_fallback_hash_and_withdrawal_reason(
    ...
    _, event = release_ledger.append_promotion_event(
        [prior], record, ReleaseStage.VERIFIED, SimpleNamespace(), {}
    )
    ...
    failures, _ = release_ledger._event_integrity_failures(
        1, broken, previous_hash="0" * 64, previous_to=None, previous_time=None
    )
