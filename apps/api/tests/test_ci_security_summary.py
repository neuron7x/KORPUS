...
def _marker(path: Path, commit: str, scanners: list[dict[str, object]]) -> None:
    path.write_text(json.dumps({"schema": "korpus.ci-scanner-result.v1", "commit_sha": commit, "scanners": scanners}))


def _run(tmp_path: Path, markers: list[Path], commit: str = "a" * 40):
    out = tmp_path / "summary.json"
    return subprocess.run([sys.executable, str(SCRIPT), *map(str, markers), "--out", str(out)], cwd=ROOT,
                          env={"CI_COMMIT_SHA": commit}, capture_output=True, text=True, check=False), out
...
def test_exact_clean_scanner_set_passes(tmp_path: Path) -> None:
    ...
    _marker(b, commit, [{"scanner": "pip-audit:runtime", "exit_code": 0}, {"scanner": "pip-audit:dev", "exit_code": 0}])
...
def test_missing_nonzero_or_stale_marker_fails(tmp_path: Path) -> None:
    ...
    _marker(a, commit, [{"scanner": "gitleaks", "exit_code": 0}])
    _marker(b, "c" * 40, [{"scanner": "pip-audit:runtime", "exit_code": 1}, {"scanner": "pip-audit:dev", "exit_code": 0}])
    run, out = _run(tmp_path, [a, b], commit)
    assert run.returncode != 0
    report = json.loads(out.read_text())
    assert report["status"] == "FAIL"
    assert {"scanner_set_exact", "all_scanners_zero", "single_commit", "current_commit"}.intersection(report["failures"])
