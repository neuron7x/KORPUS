...
def _evidence() -> tuple[dict, dict, dict, dict]:
    ...
    phase = {"requests": 10, "concurrency": 4, "p50_seconds": 0.1, "p95_seconds": 0.2, "p99_seconds": 0.3,
             "statuses": {"200": 10}, "refusal_reasons": {}, "decisions": {}}
    load = {"source_tree_sha256": "s", "release": "v", "environment_class": "PRODUCTION_LIKE",
            "cold_first_request": {"seconds": 0.1, "status": "200"},
            "load": dict(phase), "spike": dict(phase), "soak": dict(phase)}
...
def _signed(data: bytes, name: str) -> tuple[dict, str]:
    ...
    return {"algorithm": "Ed25519", "release": "v", "manifest": name,
            "manifest_sha256": hashlib.sha256(data).hexdigest(), "public_key_pem": public.decode(),
            "public_key_sha256": fingerprint, "signature_base64": base64.b64encode(key.sign(data)).decode()}, fingerprint
...
def test_pretrusted_attestations_clear_reliability_trust_boundary() -> None:
    internal, chaos, load, recovery = _evidence()
    load_bytes = (json.dumps(load, sort_keys=True) + "\n").encode(); recovery_bytes = (json.dumps(recovery, sort_keys=True) + "\n").encode()
    load_att, load_fp = _signed(load_bytes, "load-probe.json"); recovery_att, recovery_fp = _signed(recovery_bytes, "recovery-report.json")
    checks, _, _ = evaluate_attested_reliability(
        internal, chaos, load, recovery, source="s", release="v", load_bytes=load_bytes,
        recovery_bytes=recovery_bytes, load_attestation=load_att, recovery_attestation=recovery_att,
...
def test_signed_bad_load_cannot_pass_reliability_quality_predicates() -> None:
    ...
    checks, _, _ = evaluate_attested_reliability(
        ...
        load_bytes=load_bytes, recovery_bytes=recovery_bytes,
        load_attestation=load_att, recovery_attestation=recovery_att,
        trusted={load_fp, recovery_fp},
    )
