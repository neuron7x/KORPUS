...
def test_runtime_source_ref_limit_matches_frozen_evidence_contract() -> None:
    ...
    limit = contract["properties"]["provenance"]["properties"]["source_refs"]["items"]["maxLength"]
    assert limit == 1024
...
def test_capability_spec_cross_field_rules_match_runtime_validator() -> None:
    ...
    assert rules[0]["then"]["properties"]["idempotency"]["properties"]["required"]["const"] is True
    ...
    assert (
        rules[3]["if"]["properties"]["idempotency"]["properties"]["provider_key_forwarding"][
            "const"
        ]
        is True
    )
    assert rules[3]["then"]["properties"]["idempotency"]["properties"]["required"]["const"] is True

    assert rules[4]["if"]["properties"]["retry"]["properties"]["max_attempts"]["minimum"] == 2
    assert (
        rules[4]["then"]["properties"]["retry"]["properties"]["only_safe_errors"]["const"] is True
    )
    ...
    assert (
        rules[5]["then"]["properties"]["idempotency"]["properties"]["provider_key_forwarding"][
            "const"
        ]
        is True
    )
    ...
    assert (
        set(rules[6]["if"]["properties"]["evidence"]["properties"]["profile"]["enum"])
        == evidence_profiles
    )
    assert (
        rules[6]["then"]["properties"]["evidence"]["properties"]["bind_output_digest"]["const"]
        is True
    )
