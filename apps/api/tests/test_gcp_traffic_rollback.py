...
def test_service_failure_writes_error_evidence(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        rt.subprocess,
        "run",
        lambda *a, **k: SimpleNamespace(returncode=9, stdout="", stderr="denied"),
    )
    evidence = tmp_path / "api.json"
    assert (
        rt.rollback_service(
            project="p", region="r", service="korpus-api", spec="rev-a=100", evidence=evidence
        )
        is False
    )
...
def test_main_attempts_web_even_when_api_rollback_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    seen: list[str] = []
...
def test_empty_previous_traffic_is_not_applicable(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(rt.subprocess, "run", lambda *a, **k: pytest.fail("gcloud must not run"))
    evidence = tmp_path / "api.json"
    assert rt.rollback_service(
        project="p", region="r", service="korpus-api", spec="", evidence=evidence
    )
