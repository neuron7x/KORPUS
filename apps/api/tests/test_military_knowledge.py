...
def test_supersession_cycle_is_a_publication_blocker() -> None:
    graph = MilitaryKnowledgeGraph(
        nodes=(node("a"), node("b")),
        relations=(
            KnowledgeRelation(source_id="a", target_id="b", kind=KnowledgeRelationKind.SUPERSEDES, bindings=(binding(),)),
            KnowledgeRelation(source_id="b", target_id="a", kind=KnowledgeRelationKind.SUPERSEDES, bindings=(binding(),)),
        ),
    )
    assert any(item.startswith("supersession_cycle:") for item in graph.publication_violations(as_of=date(2026, 8, 20)))


def test_revision_diff_is_exact_hash_bound_and_deterministic() -> None:
    old = [RevisionUnit(0, "Крок один."), RevisionUnit(1, "Крок два."), RevisionUnit(2, "Крок три.")]
    new = [RevisionUnit(0, "Крок один."), RevisionUnit(1, "Крок два змінено."), RevisionUnit(2, "Крок три.")]
...
def test_effective_neighborhood_is_bounded_and_filters_expired_relations():
    ...
    binding = EvidenceBinding(document_id="d", version_id="v", span_ids=frozenset({"s"}), source_hash="a"*64, effective_from=date(2026,1,1))
    expired = EvidenceBinding(document_id="d", version_id="old", span_ids=frozenset({"s2"}), source_hash="b"*64, effective_until=date(2025,12,31))
    graph = MilitaryKnowledgeGraph(
        nodes=(
        ...
        relations=(
            KnowledgeRelation(source_id="a", target_id="b", kind=KnowledgeRelationKind.GOVERNS, bindings=(binding,)),
            KnowledgeRelation(source_id="b", target_id="c", kind=KnowledgeRelationKind.DEFINES, bindings=(expired,)),
        ),
    )
...
def test_precomputed_graph_index_is_exactly_equivalent_to_direct_traversal():
    ...
    binding = EvidenceBinding(document_id="d", version_id="v", span_ids=frozenset({"s"}), source_hash="c"*64)
    nodes = tuple(KnowledgeNode(id=f"n{i}", kind=KnowledgeNodeKind.TERM, label=f"N{i}", bindings=(binding,)) for i in range(8))
    relations = tuple(KnowledgeRelation(source_id=f"n{i}", target_id=f"n{i+1}", kind=KnowledgeRelationKind.DEFINES, bindings=(binding,)) for i in range(7))
    ...
    for depth in range(0, 8):
        assert index.neighborhood("n0", max_depth=depth) == effective_neighborhood(graph, "n0", as_of=observed, max_depth=depth)
