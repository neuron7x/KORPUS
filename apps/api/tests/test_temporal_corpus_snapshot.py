...
def test_approval_between_token_validation_and_retrieval_fails_closed(
    ...
    second = ingest_text(
        client, title="Race B", text="Маркер BRAVO-RACE з'являється лише після схвалення."
    )
...
def test_semantic_backfill_invalidates_an_inflight_snapshot_token(client, admin_identity) -> None:
    result = ingest_text(client)
    version_id = _version_id(result)
    approve(client, version_id)
    repository = client.app.state.repository
...
def test_monotonic_epoch_kills_release_aba_without_changing_historical_identity(
    ...
    first = ingest_text(client, title="Наказ А", text="Маркер ALPHA наказує вести журнал щоденно.")
