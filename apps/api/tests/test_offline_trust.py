...
from korpus.application.offline_trust import OfflineTrustStore, TrustedOfflineKey, verify_with_trust_store
import hashlib
...
def _key(private, key_id, start, end=None, revoked=None):
    ...
    return TrustedOfflineKey(key_id=key_id, public_key_b64=base64.b64encode(raw).decode(), valid_from=start, valid_until=end, revoked_at=revoked)


def test_key_rotation_accepts_current_key_and_rejects_retired_key():
    now = datetime(2026, 8, 20, 12, tzinfo=UTC)
    old = Ed25519PrivateKey.generate(); current = Ed25519PrivateKey.generate()
    store = OfflineTrustStore(keys=(
        _key(old, "old", now - timedelta(days=10), now - timedelta(days=1)),
        _key(current, "current", now - timedelta(days=1)),
    ))
    assert verify_with_trust_store(_signed_pack(current, "current", now), store, now=now).state is OfflinePackState.VALID
    assert verify_with_trust_store(_signed_pack(old, "old", now), store, now=now).state is OfflinePackState.SIGNATURE_INVALID


def test_revocation_is_fail_closed():
    now = datetime(2026, 8, 20, 12, tzinfo=UTC)
    key = Ed25519PrivateKey.generate()
    store = OfflineTrustStore(keys=(_key(key, "k1", now - timedelta(days=1), revoked=now - timedelta(minutes=1)),))
