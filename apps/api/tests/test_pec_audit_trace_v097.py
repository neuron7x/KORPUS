...
def test_audit_trace_is_deterministic_under_input_order():
    a = extract_audit_trace(rows(), B)
    b = extract_audit_trace(list(reversed(rows())), B)
    assert a == b and a.event_ids == ("e1", "e2")


def test_audit_trace_rejects_revision_drift():
    data = rows()
    data[0]["revision"] = "other"
    with pytest.raises(ValueError, match="revision binding mismatch"):
        extract_audit_trace(data, B)
