...
down_revision: Union[str, None] = "0002_database_defense_and_vectors"
...
def upgrade() -> None:
    ...
    op.alter_column("audit_anchor_outbox", "sequence", existing_type=sa.Integer(), type_=sa.BigInteger())
    ...
    op.execute(
        f"CREATE POLICY version_insert ON document_versions FOR INSERT WITH CHECK ({_writer()} AND {visible_version})"
    )
    op.execute(
        f"CREATE POLICY version_update ON document_versions FOR UPDATE USING ({_writer()} AND {visible_version}) "
    ...
    op.execute(
        f"CREATE POLICY evidence_span_insert ON evidence_spans FOR INSERT WITH CHECK ({_writer()} AND {visible_span})"
    )
    op.execute(
        f"CREATE POLICY evidence_span_update ON evidence_spans FOR UPDATE USING ({_writer()} AND {visible_span}) "
    ...
    op.execute(
        f"CREATE POLICY embedding_insert ON span_embeddings FOR INSERT WITH CHECK ({_writer()} AND {visible_embedding})"
    )
    op.execute(
        f"CREATE POLICY embedding_update ON span_embeddings FOR UPDATE USING ({_writer()} AND {visible_embedding}) "
...
def downgrade() -> None:
    ...
    if dialect == "postgresql":
        for table, policies in {
            ...
            "evidence_spans": ("evidence_span_delete", "evidence_span_update", "evidence_span_insert", "evidence_span_select"),
        ...
        op.alter_column("audit_heads", "sequence", existing_type=sa.BigInteger(), type_=sa.Integer())
        op.alter_column("audit_anchor_outbox", "sequence", existing_type=sa.BigInteger(), type_=sa.Integer())
        op.alter_column("audit_events", "sequence", existing_type=sa.BigInteger(), type_=sa.Integer())
