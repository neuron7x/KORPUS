...
down_revision: Union[str, None] = "0006_source_authenticity"
...
def upgrade() -> None:
    with op.batch_alter_table("document_versions") as batch:
        batch.add_column(sa.Column("content_fingerprint", sa.String(16), nullable=False, server_default="0000000000000000"))
        batch.add_column(sa.Column("near_duplicate_of_version_id", sa.String(36), sa.ForeignKey("document_versions.id", name="fk_document_versions_near_duplicate_of_version_id")))
    ...
    op.create_index("ix_document_versions_content_fingerprint", "document_versions", ["content_fingerprint"])
