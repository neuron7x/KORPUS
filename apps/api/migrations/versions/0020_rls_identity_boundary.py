...
Revision ID: 0020_rls_identity_boundary
Revises: 0019_temporal_corpus_snapshot

Портовано з GitHub-лінії `agent/issue-31-nonforgeable-rls` 01.09.2026 з ДВОМА
розширеннями, без яких межа була б дірявою саме там, де її не дивились:

1. `ingestion_jobs` у GitHub-лінії не було. Локальна лінія боронить цю таблицю
   політиками теж, і вони читали `current_setting` — тобто після «полагодження»
   лишилась би одна таблиця зі старим, підробним джерелом claim'ів.
2. claim `korpus.subject` — його ставить `ingestion_jobs.py` окремим викликом,
   а брокер GitHub-лінії його не носив. Політика порівнювала б `actor_subject`
   із порожнім рядком.

Загроза виміряна ЗАПУСКОМ на цьому дереві до правки: звичайний логін `korpus_app`
підняв собі clearance 0 → 3 і roles user → admin і ПОБАЧИВ та ЗАПИСАВ документ
grifу restricted, якого чесна особистість не бачила.
"""
...
revision: str = "0020_rls_identity_boundary"
down_revision: str | None = "0019_temporal_corpus_snapshot"
...
def _replace_policies(*, trusted: bool) -> None:
    ...
    # `ingestion_jobs` не було в GitHub-лінії. Лишити її на `current_setting` означало б
    # полагодити п'ять таблиць із шести й назвати межу непідробною: черга поглинання
    # несе, ЧИЯ це робота, і саме через неї працює воркер.
    owner = f"(ingestion_jobs.actor_subject = {subject})"
    reader = f"({owner} OR 'admin' = ANY({roles}) OR 'auditor' = ANY({roles}) OR 'worker' = ANY({roles}))"
    runner = f"('admin' = ANY({roles}) OR 'worker' = ANY({roles}))"
    op.execute(f"CREATE POLICY ingestion_job_select ON ingestion_jobs FOR SELECT USING ({reader})")
    op.execute(
        "CREATE POLICY ingestion_job_insert ON ingestion_jobs FOR INSERT WITH CHECK "
        f"({owner} AND {writer})"
    )
    op.execute(
        f"CREATE POLICY ingestion_job_update ON ingestion_jobs FOR UPDATE USING ({runner}) "
        f"WITH CHECK ({runner})"
    )
    op.execute(f"CREATE POLICY ingestion_job_delete ON ingestion_jobs FOR DELETE USING ({admin})")
...
def _create_context_functions() -> None:
    op.execute(
        ...
        CREATE FUNCTION public.korpus_bind_rls_context(
          ...
          p_subject text, p_clearance integer, p_corpora jsonb, p_classifications jsonb,
        ...
        BEGIN
          ...
          INSERT INTO public.korpus_rls_context AS c (
            backend_pid, transaction_id, session_login, subject, clearance,
            corpora, classifications, compartments, roles, bound_at
          ) VALUES (
            p_backend_pid, p_transaction_id, p_session_login, p_subject, p_clearance,
    ...
    op.execute(
        "REVOKE ALL ON FUNCTION public.korpus_bind_rls_context(integer,bigint,name,text,integer,jsonb,jsonb,jsonb,jsonb) FROM PUBLIC"
    )
...
def downgrade() -> None:
    ...
    for name in ("roles", "compartments", "classifications", "corpora", "clearance", "subject"):
        op.execute(f"DROP FUNCTION IF EXISTS public.korpus_rls_{name}()")
    op.execute(
        "DROP FUNCTION IF EXISTS public.korpus_bind_rls_context(integer,bigint,name,text,integer,jsonb,jsonb,jsonb,jsonb)"
    )
