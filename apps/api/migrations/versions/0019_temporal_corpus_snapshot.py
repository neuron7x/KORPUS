...
revision: str = "0019_temporal_corpus_snapshot"
down_revision: str | None = "0018_operational_competencies"
...
def upgrade() -> None:
    ...
    # Типізована операція alembic замість сирого SQL, і це не стиль. Правило
    # «лише розширення» судить ДІЄСЛОВО оператора; сирий `INSERT` через `op.execute`
    # незмінно виглядав би як мутація даних у міграції, хоч сіє один керуючий рядок.
    # `bulk_insert` каже те саме, і каже це так, що гейт може прочитати.
    op.bulk_insert(
        sa.table(
            "corpus_state_epoch",
