  ...
  try {
    ...
    await runCase("mobile_viewport_has_no_horizontal_overflow", async () => {
      ...
      for (const viewport of [{width:320,height:568}, {width:390,height:844}, {width:844,height:390}]) {
        ...
        const state = await cdp.evaluate(`(() => { const box=selector=>{const r=document.querySelector(selector).getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height};}; const more=document.querySelector(".mobile-more"); more.open=true; const secondary=[...more.querySelectorAll("a")].filter(node=>getComputedStyle(node).display!=="none").length,mobileNew=box("#conversation-new-mobile"); more.open=false; return {viewport:innerWidth,scroll:document.documentElement.scrollWidth,composer:box(".composer"),query:box("#query"),theme:box("#theme-toggle"),dock:box(".mobile-nav"),railVisible:getComputedStyle(document.querySelector(".left-rail")).display!=="none",primaryVisible:getComputedStyle(document.querySelector(".primary-nav")).display!=="none",secondary,mobileNew,nav:[...document.querySelectorAll(".mobile-nav>a")].filter(node=>getComputedStyle(node).display!=="none").map(node=>node.getBoundingClientRect().height)}; })()`);
