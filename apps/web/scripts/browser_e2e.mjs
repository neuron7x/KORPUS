...
async function main() {
  ...
  try {
    ...
    await runCase("combat_theme_is_reversible_and_accessible", async () => {
      ...
      const radar = await cdp.evaluate(`(() => { dispatchEvent(new CustomEvent("korpus:radar",{detail:{state:"answered",stage:3,coverage:.86,sources:[{source_hash:"${"a".repeat(64)}"},{source_hash:"${"b".repeat(64)}"}]}})); const canvas=document.getElementById("combat-signal-field"); return {status:document.getElementById("combat-radar-status")?.textContent,contacts:canvas?.dataset.contacts,stage:canvas?.dataset.stage}; })()`);
      assert(radar.status.includes("2 ДЖ.") && radar.status.includes("86%") && radar.contacts === "2" && radar.stage === "3", "combat radar is decorative rather than evidence-bound");
