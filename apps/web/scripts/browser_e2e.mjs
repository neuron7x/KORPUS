...
async function main() {
  ...
  try {
    ...
    await runCase("combat_theme_is_reversible_and_accessible", async () => {
      ...
      const combat = await cdp.evaluate(`(() => { const button=document.getElementById("theme-toggle"); const display=s=>getComputedStyle(document.querySelector(s)).display; const box=s=>document.querySelector(s).getBoundingClientRect(); const canvasNode=document.getElementById("combat-signal-field"),canvas=box("#combat-signal-field"),promptNode=document.querySelector(".empty-chat h2"),prompt=box(".empty-chat h2"),promptStyle=getComputedStyle(promptNode); return {theme:document.documentElement.dataset.theme, pressed:button.getAttribute("aria-pressed"), label:button.getAttribute("aria-label"), stylesheet:document.getElementById("combat-theme")?.getAttribute("href"), canvas:canvasNode?.tagName,topline:display(".chat-topline"),trace:display(".evidence-trace"),empty:display(".empty-chat"),emptyCopy:promptNode?.textContent,promptColor:promptStyle.color,promptFill:promptStyle.webkitTextFillColor,promptLineHeight:promptStyle.lineHeight,centerDelta:Math.abs((canvas.left+Number(canvasNode.dataset.centerX))-(prompt.left+prompt.width/2))}; })()`);
      ...
      assert(combat.promptColor === combat.promptFill && Number.parseFloat(combat.promptLineHeight) >= 21, `combat prompt is not rendered as solid readable copy: ${JSON.stringify(combat)}`);
    ...
    await runCase("mobile_viewport_has_no_horizontal_overflow", async () => {
      ...
      for (const viewport of [{width:320,height:568}, {width:390,height:844}, {width:844,height:390}]) {
        ...
        await cdp.evaluate(`document.documentElement.dataset.theme === "combat" || document.getElementById("theme-toggle").click()`);
        await waitFor(cdp, 'document.documentElement.dataset.theme === "combat" && document.getElementById("combat-signal-field")', "mobile combat layout");
        const state = await cdp.evaluate(`(() => { const box=selector=>{const r=document.querySelector(selector).getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height};}; const more=document.querySelector(".mobile-more"); more.open=true; const secondary=[...more.querySelectorAll("a")].filter(node=>getComputedStyle(node).display!=="none").length,mobileNew=box("#conversation-new-mobile"); more.open=false; const prompt=box(".empty-chat h2"),actions=box(".quick-actions"),ask=document.getElementById("ask-section"),canvas=document.getElementById("combat-signal-field"),canvasBox=box("#combat-signal-field"),radarBottom=canvasBox.top+Number(canvas.dataset.centerY)+Number(canvas.dataset.radius); return {viewport:innerWidth,scroll:document.documentElement.scrollWidth,composer:box(".composer"),query:box("#query"),theme:box("#theme-toggle"),dock:box(".mobile-nav"),prompt,actions,radarGap:prompt.top-radarBottom,askPosition:getComputedStyle(ask).position,railVisible:getComputedStyle(document.querySelector(".left-rail")).display!=="none",primaryVisible:getComputedStyle(document.querySelector(".primary-nav")).display!=="none",secondary,mobileNew,nav:[...document.querySelectorAll(".mobile-nav>a")].filter(node=>getComputedStyle(node).display!=="none").map(node=>node.getBoundingClientRect().height)}; })()`);
        ...
        assert(state.query.height > 0, `${viewport.width}x${viewport.height}: query field is not visible ${JSON.stringify(state)}`);
        assert(!state.railVisible && !state.primaryVisible && state.dock.height > 0, `${viewport.width}x${viewport.height}: desktop chrome competes with the mobile task surface`);
        assert(state.composer.height <= 140, `${viewport.width}x${viewport.height}: composer consumes ${state.composer.height}px of the viewport`);
        assert(state.askPosition === "relative", `${viewport.width}x${viewport.height}: ready composer escaped document flow (${state.askPosition})`);
        assert(state.radarGap >= 20, `${viewport.width}x${viewport.height}: radar collides with prompt (${state.radarGap}px)`);
        assert(state.prompt.bottom <= state.actions.top + 1, `${viewport.width}x${viewport.height}: prompt overlaps quick actions ${JSON.stringify({prompt:state.prompt,actions:state.actions})}`);
        await cdp.evaluate(`(() => { document.documentElement.style.scrollBehavior="auto"; scrollTo(0,document.documentElement.scrollHeight); })()`);
        await new Promise(resolve => setTimeout(resolve, 50));
        const scrolled = await cdp.evaluate(`(() => { const box=selector=>{const r=document.querySelector(selector).getBoundingClientRect();return {top:r.top,bottom:r.bottom};}; return {composer:box(".composer"),dock:box(".mobile-nav")}; })()`);
        assert(scrolled.composer.bottom <= scrolled.dock.top + 1, `${viewport.width}x${viewport.height}: scrolled composer is obscured by navigation ${JSON.stringify(scrolled)}`);
        assert(state.secondary >= 1, `${viewport.width}x${viewport.height}: authorized secondary routes are unreachable`);
        assert(state.mobileNew.height >= 44, `${viewport.width}x${viewport.height}: new conversation is not touch accessible`);
