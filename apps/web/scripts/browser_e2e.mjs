...
async function main() {
  ...
  try {
    ...
    await runCase("combat_theme_is_reversible_and_accessible", async () => {
      ...
      const combat = await cdp.evaluate(`(() => { const button=document.getElementById("theme-toggle"); const display=s=>getComputedStyle(document.querySelector(s)).display; return {theme:document.documentElement.dataset.theme, pressed:button.getAttribute("aria-pressed"), label:button.getAttribute("aria-label"), stylesheet:document.getElementById("combat-theme")?.getAttribute("href"), canvas:document.getElementById("combat-signal-field")?.tagName,topline:display(".chat-topline"),trace:display(".evidence-trace"),empty:display(".empty-chat"),emptyCopy:document.querySelector(".empty-chat h2")?.textContent}; })()`);
