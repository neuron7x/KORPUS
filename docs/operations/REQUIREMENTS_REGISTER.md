...
| `repo.version.api_pyproject` | api_pyproject declares release 6.10.1 | release identity and its derivative surfaces must agree; one mismatch means the artefacts describe different builds |
| `repo.version.readme_header` | readme_header declares release 6.10.1 | release identity and its derivative surfaces must agree; one mismatch means the artefacts describe different builds |
| `repo.version.release_identity` | release_identity declares release 6.10.1 | release identity and its derivative surfaces must agree; one mismatch means the artefacts describe different builds |
| `repo.version.runtime_dunder` | runtime_dunder declares release 6.10.1 | release identity and its derivative surfaces must agree; one mismatch means the artefacts describe different builds |
| `repo.version.web_package` | web_package declares release 6.10.1 | release identity and its derivative surfaces must agree; one mismatch means the artefacts describe different builds |
