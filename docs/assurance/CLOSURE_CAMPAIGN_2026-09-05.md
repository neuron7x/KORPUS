...
`local-summary.json` і називає себе (`class: LOCAL_SCANNERS`); клас підсумку
перевіряється окремою назвою (`security_summary_is_ci_aggregate`).

## 10. Кампанія, що перезнімала не той файл

`RUNNERS["redteam"]` викликав виробника, який пише `redteam-gate.json`, тоді як
предикат читає `redteam_internal-gate.json`. Кампанія відпрацьовувала, звітувала про
